...
typedef enum
{
    ...
    CodecSpecifyCodedMemoryPartition,
...
typedef struct CodecBufferState_s
{
    ...
    unsigned char                *BufferMacroblockStructurePointer;     // Only for some types of video
...
protected:
    ...
    CodecStatus_t           TestMarkerFramePassOn(      void );
...
public:
    ...
    virtual CodecStatus_t   GetCodedFrameBufferPool(    BufferPool_t             *Pool,
						unsigned int             *MaximumCodedFrameSize );   // WMA/OGG need to extend
