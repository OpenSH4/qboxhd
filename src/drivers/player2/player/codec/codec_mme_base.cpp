...
//
    ...
    CodedFrameMemory[CachedAddress]     = NULL;
    CodedFrameMemory[UnCachedAddress]   = NULL;
    ...
    Configuration.CodedMemorySize                       = 0x100000;
    Configuration.MaximumCodedFrameSize                 = 0x10000;
#ifdef __TDT__
...
//Dagobert
    strcpy( Configuration.CodedMemoryPartitionName, "BPA2_Region1" );
#else
    strcpy( Configuration.CodedMemoryPartitionName, "BPA2_Region0" );
#endif
    ...
    Configuration.TrickModeParameters.MaximumNormalDecodeRate           = 1;
    Configuration.TrickModeParameters.MaximumSubstandardDecodeRate      = 0;
    Configuration.TrickModeParameters.DefaultGroupSize                  = 1;
    Configuration.TrickModeParameters.DefaultGroupReferenceFrameCount   = 0;
    Configuration.TrickModeParameters.AutoAdjustDecodeRates             = false;
...
CodecStatus_t   Codec_MmeBase_c::Reset( void )
{
    ...
    if( CodedFrameMemory[CachedAddress] != NULL )
    {
#if __KERNEL__
						...
						void          *ParameterBlock )
{
	...
	case CodecSpecifyCodedMemoryPartition:
	    strcpy( Configuration.CodedMemoryPartitionName, CodecParameterBlock->PartitionName );
	    report( severity_info, "Codec_MmeBase_c::SetModuleParameters - Specified coded memory partition '%s'.\n", CodecParameterBlock->PartitionName );
	    break;
...
CodecStatus_t   Codec_MmeBase_c::GetCodedFrameBufferPool(
						BufferPool_t     *Pool,
...
#if __KERNEL__
	AStatus = PartitionAllocatorOpen( &CodedFrameMemoryDevice, Configuration.CodedMemoryPartitionName, Configuration.CodedMemorySize, true );
	if( AStatus != allocator_ok )
	{
	    report( severity_error, "Codec_MmeBase_c::GetCodedFrameBufferPool(%s) - Failed to allocate memory\n", Configuration.CodecName );
	    return PlayerInsufficientMemory;
	}

	CodedFrameMemory[CachedAddress]         = AllocatorUserAddress( CodedFrameMemoryDevice );
	CodedFrameMemory[UnCachedAddress]       = AllocatorUncachedUserAddress( CodedFrameMemoryDevice );
	CodedFrameMemory[PhysicalAddress]       = AllocatorPhysicalAddress( CodedFrameMemoryDevice );
#else
	static unsigned char    Memory[4*1024*1024];

	CodedFrameMemory[CachedAddress]         = Memory;
	CodedFrameMemory[UnCachedAddress]       = NULL;
	CodedFrameMemory[PhysicalAddress]       = Memory;
	Configuration.CodedMemorySize           = 4*1024*1024;
#endif

//

	Status  = BufferManager->CreatePool( &CodedFramePool, CodedFrameBufferType, Configuration.CodedFrameCount, Configuration.CodedMemorySize, CodedFrameMemory );
	if( Status != BufferNoError )
	{
	    report( severity_error, "Codec_Dummy_c::GetCodedFrameBufferPool(%s) - Failed to create the pool.\n", Configuration.CodecName );
	    return PlayerInsufficientMemory;
	}
    }
    ...
    *Pool                       = CodedFramePool;
    *MaximumCodedFrameSize      = Configuration.MaximumCodedFrameSize;
...
{
    *TrickModeParameters        = &Configuration.TrickModeParameters;
    return CodecNoError;
}
...
Buffer_t        AttachedCodedDataBuffer;
	...
	for( j=0; j<Context->ReferenceFrameList[i].EntryCount; j++ )
	    if( Context->ReferenceFrameList[i].EntryIndicies[j] != INVALID_INDEX )
		DecrementReferenceCount( Context->ReferenceFrameList[i].EntryIndicies[j] );
...
#else
        report(severity_fatal,"Error - NULL buffer index %d\n",BufferIndex);
