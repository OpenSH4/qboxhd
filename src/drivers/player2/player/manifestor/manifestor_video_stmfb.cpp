...
extern "C" {
  ...
  extern stm_display_buffer_t*       ManifestorLastDisplayedBuffer;
...
#ifdef __TDT__
#ifdef UFS922
	cpp_remove_e2_procs((char*)"stb/video/plane/dei_fmd", videoStmfb_dei_set_fmd_read, videoStmfb_dei_set_fmd_write);
	cpp_remove_e2_procs((char*)"stb/video/plane/dei_mode", videoStmfb_dei_set_mode_read, videoStmfb_dei_set_mode_write);
	cpp_remove_e2_procs((char*)"stb/video/plane/dei_ctrl", videoStmfb_dei_set_ctrl_read, videoStmfb_dei_set_ctrl_write);
#endif
    cpp_remove_e2_procs((char*)"stb/video/plane/psi_brightness", videoStmfb_psi_brightness_read, videoStmfb_psi_brightness_write);
	cpp_remove_e2_procs((char*)"stb/video/plane/psi_saturation", videoStmfb_psi_saturation_read, videoStmfb_psi_saturation_write);
	cpp_remove_e2_procs((char*)"stb/video/plane/psi_contrast", videoStmfb_psi_contrast_read, videoStmfb_psi_contrast_write);
	cpp_remove_e2_procs((char*)"stb/video/plane/psi_tint", videoStmfb_psi_tint_read, videoStmfb_psi_tint_write);
#endif
...
ManifestorStatus_t Manifestor_VideoStmfb_c::Reset (void)
{
    ...
    {
        memset ((void*)&DisplayBuffer[i], 0, sizeof(stm_display_buffer_t));
                                                                  ...
                                                                  unsigned int            OutputId)
{
    ...
    MANIFESTOR_TRACE("DisplayDevice %p, PlaneId %x\n", DisplayDevice, PlaneId);
    if (Plane == NULL)
    ...
    if (CurrentMode != NULL)
    {
        ...
        MANIFESTOR_TRACE("Width %d, height %d, FrameRate %d.%06d, Progressive %d\n",
                                                                ...
                                                                struct BufferStructure_s*       BufferStructure)
{
    ...
    {
//        MANIFESTOR_TRACE("Setting output clock adjustment to %d parts per million\n", Adjustment);
    ...
    DisplayBuff->src.ulConstAlpha               = (Visible) ? 255 : 0;
    ...
    switch( VideoParameters->Content.ColourMatrixCoefficients )
    {
        ...
        case MatrixCoefficients_SMPTE_240M:
                ...
                FirstFieldFlags                |= STM_PLANE_SRC_COLORSPACE_709;
                break;
    ...
    //{{{  check pan/scan info
    {
        ...
        //for (i = 0; i < VideoOutputTiming->PanScan.Count; i++)
        //{
        //    MANIFESTOR_TRACE ("Pan scan (%d, %d), Horiz %d, vert %d\n", i, VideoOutputTiming->PanScan.DisplayCount[i],
    ...
    for (i = 0, Status = 0; (i < Count) && (Status == 0); i++)
    {
        ...
        if (Status != 0)
        {
            ...
            MANIFESTOR_FATAL ("Failed to queue buffer %d to display\n", BufferIndex);
                                                                ...
                                                                struct BufferStructure_s*       BufferStructure)
{
    ...
    MANIFESTOR_TRACE("Queueing initial frame %d\n", BufferIndex);
    InitialFrameState                           = InitialFrameQueued;
    if (stm_display_plane_queue_buffer (Plane, DisplayBuff) == 0)
        return ManifestorNoError;
...
ManifestorStatus_t Manifestor_VideoStmfb_c::UpdateDisplayWindows (void)
{
    ...
    MANIFESTOR_TRACE("%d,%d, %dx%d, Dest %d,%d, %dx%d\n",
...
ManifestorStatus_t Manifestor_VideoStmfb_c::Enable (void)
{
    //MANIFESTOR_DEBUG ("\n");
    Visible     = true;
#warning "** Leftout until new stmfb available stm_display_plane_show (Plane);"
...
ManifestorStatus_t Manifestor_VideoStmfb_c::Disable (void)
{
    //MANIFESTOR_DEBUG ("\n");
    Visible             = false;
#warning "** Leftout until new stmfb available stm_display_plane_hide (Plane);"
                                                ...
                                                TIME64                  VsyncTime)
{
    ...
    ManifestorLastDisplayedBuffer = &DisplayBuffer[Buffer->BufferIndex];
...
#define RELAY_CRC
    ...
    Manifestor->DoneCallback (StreamBuffer, Data ? Data->vsyncTime : 0);
