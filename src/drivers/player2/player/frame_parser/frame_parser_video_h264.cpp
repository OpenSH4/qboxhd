...
//
    ...
    {
	memset( &SliceHeaderRead, 0x00, sizeof(H264SliceHeader_t) );
	SeenAnIDR			= false;
	BehaveAsIfSeenAnIDR		= false;
...
//
	...
	Bits.FlushUnseen( 24 );			// Lose the 00 00 01

	MarkerBit(0);				// forbidden_zero
	nal_ref_idc		= Bits.Get(2);
	nal_unit_type		= Bits.Get(5);
...
//
	...
	switch( nal_unit_type )
	{
	    ...
	    case NALU_TYPE_SLICE:
			...
			SliceHeader	= &SliceHeaderRead;
			Status 		= ReadNalSliceHeader();
#if 0
report( severity_info, "Slice type = %d\n", SliceHeader->slice_type );
#endif
			if( Status == FrameParserNoError )
			{
	    ...
	    case NALU_TYPE_SEI:
			if( i < (StartCodeList->NumberOfStartCodes-1) )
			    MaximumSize	= ExtractStartCodeOffset(StartCodeList->StartCodes[i+1]) - ExtractStartCodeOffset(Code) - 4;
			else
			    MaximumSize	= (BufferLength + 1) - ExtractStartCodeOffset(Code) - 4;

			Status = ReadNalSupplementalEnhancementInformation( MaximumSize );
			break;
...
//
	...
	if( Status != FrameParserNoError )
	{
		...
		if(Status==FrameParserCodecError)
    		return FrameParserCodecError;/* So exit if there is a lot of error maybe for wrong decode */
#endif
	}
    }
...
//
    ...
    {
	CheckAntiEmulationBuffer( SCALING_LIST_ANTI_EMULATION_REQUEST );
...
//
    ...
    CheckAntiEmulationBuffer( HRD_PARAMETERS_ANTI_EMULATION_REQUEST );
...
//
    ...
    CheckAntiEmulationBuffer( VUI_PARAMETERS_ANTI_EMULATION_REQUEST );
...
//
    ...
    CheckAntiEmulationBuffer( SPS_EXTENSION_ANTI_EMULATION_REQUEST );
...
{
FrameParserStatus_t	  Status;
H264SliceHeader_t	 *Header = SliceHeader;
...
unsigned int		  slice_type;
	...
	if( Header->Valid &&
	    (Header->SequenceParameterSet->profile_idc == H264_PROFILE_IDC_BASELINE) &&
...
//
    ...
    if( slice_type != Header->slice_type )
	...
	return Header->Valid ? FrameParserNoError : FrameParserError;
    }
    ...
    memset( Header, 0x00, sizeof(H264SliceHeader_t) );
...
#if defined CONFIG_SH_QBOXHD_1_0 || defined CONFIG_SH_QBOXHD_MINI_1_0
		return FrameParserCodecError; /* For error_h264_header */
#else
		break;
#endif
...
FrameParserStatus_t   FrameParser_VideoH264_c::ReadNalSupplementalEnhancementInformation( unsigned int	MaximumSize )
{
FrameParserStatus_t	 Status;
unsigned int    	 PayloadType;
unsigned int    	 PayloadSize;
...
//
	...
	CheckAntiEmulationBuffer( PayloadSize + 4 );
...
//
    ...
    SliceHeader->ExtendedPicOrderCnt	= SeenDpbValue ?
    ...
    if( MemoryControlClearSeen && !SeenDpbValue )
    {
	PicOrderCntOffset		= PicOrderCntOffset + PicOrderCntOffsetAdjust;	// Add 2^32 to cater for negative pic order counts in the next sequence.
    }
...
bool			ApplyTwoRefTestForBframes;
    ...
    //
    // Caluclate the reference picture lists on the first slice,
...
FrameParserStatus_t   FrameParser_VideoH264_c::ForPlayUpdateReferenceFrameList( void )
{
    ...
    if( ParsedVideoParameters->FirstSlice && ParsedFrameParameters->ReferenceFrame )
    {
	SliceHeader		= &((H264FrameParameters_t *)(ParsedFrameParameters->FrameParameterStructure))->SliceHeader;
    	NumReferenceFrames	= SliceHeader->SequenceParameterSet->num_ref_frames;
...
MatrixCoefficientsType_t		  MatrixCoefficients;
    ...
    FirstSlice			= (SliceHeader->first_mb_in_slice == 0);
	...
	if( Status != FrameParserNoError )
	{
    	...
    	//
    	// Obtain and fill out a frame parameters structure
    	//
   	...
   	memcpy( &FrameParameters->SliceHeader, SliceHeader, sizeof(H264SliceHeader_t) );
...
report( severity_info, "NickQ NonPairedOutput - (%d %d) (%d %d) (%d %d)\n",
    ...
    //
    // Monitor slice type variability within frames
    //

    if( FirstDecodeOfFrame )
	FrameSliceType			= SliceHeader->slice_type;
    else if( SliceHeader->slice_type != FrameSliceType )
	FrameSliceTypeVaries		= true;
    ...
    ParsedFrameParameters->IndependentFrame				= ParsedFrameParameters->KeyFrame || ((SliceType == SliceTypeI) && !FrameSliceTypeVaries);
    ParsedFrameParameters->ReferenceFrame                               = (SliceHeader->nal_ref_idc != 0);

//

    ParsedVideoParameters->SliceType					= SliceType;
    ParsedVideoParameters->FirstSlice					= FirstSlice;
    ...
    ParsedVideoParameters->Content.PixelAspectRatio                     = (SPS->vui_seq_parameters.aspect_ratio_idc != H264_ASPECT_RATIO_EXTENDED_SAR) ?
										H264AspectRatios(SPS->vui_seq_parameters.aspect_ratio_idc) :
										Rational_t( SPS->vui_seq_parameters.sar_width, SPS->vui_seq_parameters.sar_height );
    ...
    else if( !inrange( ParsedVideoParameters->Content.FrameRate, 8, 120 ) )
    ...
    {
	ParsedFrameParameters->NewFrameParameters		= true;		// Set this to force processing,
...
bool            Different;
    ...
    //
    // If we haven't ovwritten any, and the pointers are unchanged, then they must the same.
    //

    if( !ReadNewParameterSet &&
...
//
    ...
    //
    // Otherwise we need to do straightforward memory
    // compares of the actual substructures.
    //
...
//
	...
	else
	{
	    ...
	    {
		case 0:	CropUnitX	= 1;
			CropUnitY	= 2 - SliceHeader->SequenceParameterSet->frame_mbs_only_flag;
			break;

		default:
		case 1:	CropUnitX	= 2;
			CropUnitY	= 2 * (2 - SliceHeader->SequenceParameterSet->frame_mbs_only_flag);
			break;


		case 2:	CropUnitX	= 2;
			CropUnitY	= 1 * (2 - SliceHeader->SequenceParameterSet->frame_mbs_only_flag);
			break;
	    ...
	    {
#if 0
		Left				= 16 * CropUnitX * SliceHeader->SequenceParameterSet->frame_cropping_rect_left_offset +
						  SEIPanScanRectangle->pan_scan_rect_left_offset[i];
		Right				= 16 * (16 * (SliceHeader->SequenceParameterSet->pic_width_in_mbs_minus1 + 1)  -
							CropUnitX * SliceHeader->SequenceParameterSet->frame_cropping_rect_right_offset) +
						  SEIPanScanRectangle->pan_scan_rect_right_offset[i];

		Top				= 16 * CropUnitY * SliceHeader->SequenceParameterSet->frame_cropping_rect_top_offset +
						  SEIPanScanRectangle->pan_scan_rect_top_offset[i];
		Bottom				= 16 * (16 * FrameHeightInMbs -
							CropUnitY * SliceHeader->SequenceParameterSet->frame_cropping_rect_bottom_offset) +
