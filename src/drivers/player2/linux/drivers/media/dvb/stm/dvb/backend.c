	...
	if (*Stream == NULL)
	{
		...
		*Stream     = kzalloc (sizeof(struct StreamContext_s), GFP_KERNEL);
		...
		else
		{
			...
			{
				//BACKEND_DEBUG("-----> Stream is part of a demux\n");
...
/*}}}  */
/*{{{  PlaybackSetPlayInterval*/
int PlaybackSetPlayInterval (struct PlaybackContext_s*  Playback,
                             dvb_play_interval_t*       PlayInterval)
{
	...
	Result      = Backend->Ops->playback_set_play_interval (Playback->Handle, (play_interval_t*)PlayInterval);
	if (Result < 0)
