 ...
 *
 * Code to initialize EMBX2 on an MB618.
 */

#define MAILBOX0	0xfe211000	/* Video LX */
#define MAILBOX1	0xfe212000	/* Audio LX */
...
static void embxPlatformInit(void)
{
	...
	if (err != EMBX_ALREADY_INITIALIZED) {
		err = EMBX_Mailbox_Register((void *) MAILBOX0, OS21_INTERRUPT_MB_LX_DPHI, 0, 
					    EMBX_MAILBOX_FLAGS_SET2);
		assert (EMBX_SUCCESS == err); 

		err = EMBX_Mailbox_Register((void *) MAILBOX1, OS21_INTERRUPT_MB_LX_AUDIO, 0, 
					    EMBX_MAILBOX_FLAGS_SET2);
		assert (EMBX_SUCCESS == err); 
...
static void embxPlatformInit(void)
{
	...
	assert(err && out == sizeof(sysintr));
        ...
        err = EMBX_Mailbox_Register((void *) MAILBOX0, sysintr, 0, 
	 ...
	 */
	err = EMBX_Mailbox_Register((void *) MAILBOX1, -1, -1, EMBX_MAILBOX_FLAGS_LOCKS);
	assert (EMBX_SUCCESS == err); 
...
#if defined(__OS21__)
#include <os21/st200/sti7111_audio.h>
#endif
...
static void embxPlatformInit(void)
{
	...
	err = EMBX_Mailbox_Register((void *) MAILBOX0, -1, -1, 0);
	assert (EMBX_SUCCESS == err);
        err = EMBX_Mailbox_Register((void *) MAILBOX1, OS21_INTERRUPT_MBOX_SH4, 0, 
