/***********************************************************************
 *
 * File: stgfb/Linux/video/hdmiedid.c
...
static int stmhdmi_read_edid_block0(struct stm_hdmi *hdmi, edid_block_t rawedid)
{
  ...
  /*
   * Add in VGA, if specified in established timings, after querying all
   * other modes. This is the last resort fallback.
   */
        ...
        for(offset=0;offset<len;offset++)
        {
          ...
          if(!stmhdmi_cea_process_video_short_codes(hdmi, cea_mode))
          {
            ...
            switch(cea_mode)
            {
              case 6 ... 7:
              case 21 ... 22:
                hdmi->edid_info.cea_capabilities |= STM_CEA_CAPS_PIXEL_DOUBLE_SD;
                break;
              case 10 ... 11:
              case 25 ... 26:
                hdmi->edid_info.cea_capabilities |= STM_CEA_CAPS_PIXEL_QUAD_SD;
                break;
              case 14 ... 15:
              case 29 ... 30:
                hdmi->edid_info.cea_capabilities |= STM_CEA_CAPS_PIXEL_DOUBLE_ED;
                break;
              case 35 ... 38:
                hdmi->edid_info.cea_capabilities |= STM_CEA_CAPS_PIXEL_QUAD_ED;
                break;
            }
          }
          else
