...
static int cx24116_send_diseqc_msg(struct dvb_frontend *, struct dvb_diseqc_master_cmd *);
...
#else
    /* Write the firmware in small transactions */
    len=fw->size;
        n=0;
//#define MAX_TSIZE 1       /* Testing: Write transactions of 1 byte */
#define MAX_TSIZE 2048
    do {
        cx24116_writeregN(state, 0xF7, fw->data + n, (len > MAX_TSIZE ? MAX_TSIZE : len));
        //cx24116_writereg(state, 0xF7, *(fw->data + n));
        n += MAX_TSIZE;
        len -= MAX_TSIZE;
    } while (len > 0);
#endif
...
#ifdef RETRY_LOCK
	/* Send the last diseqc msg again when we don't have signal
	 * needed when the antenna cable is connected after a removal */
	if (last_diseqc_master_cmd.msg_len != 0) {
		if (*status <= 0x01)
			status_cnt++;
		if (status_cnt == 20) {
			cx24116_send_diseqc_msg(fe, &last_diseqc_master_cmd);
			status_cnt = 0;
		}
	}
#endif
...
#if defined(CONFIG_SH_QBOXHD_1_0)
...
static int cx24116_get_frontend(struct dvb_frontend *fe,
	...
	struct dvb_frontend_parameters *p)
{
	...
	do {
		...
		//cx24116_writereg(state, CX24116_REG_SSTATUS, 0x80);
		//msleep(5);
...
{
    dprintk("cx24116 loaded\n");
    return 0;
}

void __exit cx24116_exit(void)
{
   dprintk("cx24116 unloaded\n");
}
