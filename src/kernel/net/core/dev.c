...
#if defined(CONFIG_VMII) || defined(CONFIG_VMII_MODULE)
struct sk_buff *(*vmii_handle_frame_hook)(struct sk_buff *pskb, struct net_device *dev,
			struct packet_type *pt, struct net_device *orig_dev);
EXPORT_SYMBOL(vmii_handle_frame_hook);

static inline struct sk_buff *handle_vmii(struct sk_buff *skb,
					struct packet_type **pt_prev, int *ret,
					struct net_device *orig_dev)
{
	if (*pt_prev) {
	...
	if (vmii_handle_frame_hook)
		return vmii_handle_frame_hook(skb, skb->dev, *pt_prev, orig_dev);

	return skb;
}
#else
#define handle_vmii(skb, pt_prev, ret, orig_dev)	(skb)
#endif
...
ncls:
	...
	if (!skb)
		goto out;
	skb = handle_vmii(skb, &pt_prev, &ret, orig_dev);
	if (!skb)
		goto out;
