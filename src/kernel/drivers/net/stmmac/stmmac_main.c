...
int wifi_connect = 0;		/* It is used only for the first time, after power on */
extern unsigned char link;	/* 0->undefined, 1->no connect, 2-> connect */
...
//	printk(KERN_DEBUG "stmmac_adjust_link: start\n");
...
/* Forced the SPEED to 100Mb (internal mac of ST) */
#ifdef CONFIG_SH_QBOXHD_1_0
				printk("Forced the speed of internal mac\n");
				priv->fix_mac_speed(priv->bsp_priv,SPEED_100);
#else
...
/* No re-write of ETH_MAC_CTRL register (it is setm in u-boot) */
...
#ifdef CONFIG_SH_QBOXHD_1_0
	if(wifi_connect==1)	/* WiFi connect */
	{
		printk("WiFi is present: 100/Full\n");
		wifi_connect=2;	/* WiFi connect and message is printed */
	}
#endif
	spin_unlock_irqrestore(&priv->lock, flags);

	DBG(probe, DEBUG, "stmmac_adjust_link: exiting\n");
//	printk(KERN_DEBUG "stmmac_adjust_link: exiting\n");
...
#ifdef CONFIG_SH_QBOXHD_1_0
	if( (link == 1)  && (wifi_connect==0) )  /* No link */
		wifi_connect = 1;
#endif /* CONFIG_SH_QBOXHD_1_0 */
	return count;
}
