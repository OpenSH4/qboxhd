...
#ifdef CONFIG_PM
static int
ehci_hcd_st40_driver_suspend(struct platform_device *pdev, pm_message_t state)
{
...
static int ehci_hcd_st40_driver_resume(struct platform_device *pdev)
{
	if (device_may_wakeup(&pdev->dev))
		disable_irq_wake(pdev->resource[1].start);
	return 0;
}
#endif
...
#ifdef CONFIG_PM
	.suspend = ehci_hcd_st40_driver_suspend,
	.resume = ehci_hcd_st40_driver_resume,
#endif
	.driver = {
		.name = "stm-ehci",
