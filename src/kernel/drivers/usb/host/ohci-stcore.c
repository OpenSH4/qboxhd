...
#ifdef CONFIG_PM
static int ohci_hcd_stm_suspend(struct platform_device *pdev, pm_message_t state)
{
...
static int ohci_hcd_stm_resume(struct platform_device *pdev)
{
	if (device_may_wakeup(&pdev->dev))
...
#ifdef CONFIG_PM
	.suspend = ohci_hcd_stm_suspend,
	.resume = ohci_hcd_stm_resume,
#endif
};
