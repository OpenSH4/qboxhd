...
static char __attribute__((unused)) *dma_mode_string (void)
{
	switch (dma_mode) {
	case 0:
		return "SLOW DREQ";
		...
		u8 *buf, struct net2272_request *req, unsigned max)
{
...
#if 0
	VDEBUG (dev, "write packet %s req %p max %u len %u avail %u\n",
	...
	while (likely (count >= 2)) {
		...
		net2272_write_data(dev, *bufp++);
		count -= 2;
	}
	buf = (u8 *)bufp;

	if (unlikely (count)) {
		net2272_writeb_data(dev, *buf);		/* write final byte */
...
top: 
	...
	while (!(net2272_ep_read (ep, EP_STAT0) & (1 << BUFFER_FULL))) {
		...
		max = net2272_ep_read (ep, EP_AVAIL0) |
			(net2272_ep_read (ep, EP_AVAIL1) << 8);
...
static noinline
void fast_read_packet(u32 __iomem *fifo, unsigned long *buf, int count)
{
	asm volatile (
		"mov.l	r8,@-r15\n\t"
		"mov.l	r9,@-r15\n\t"
...
static void do_read_packet(struct net2272 *dev, unsigned char *buf, int len)
{
	if (dev->shift_addr > L1_CACHE_SHIFT && len >= L1_CACHE_BYTES) {
		u32 __iomem *fifo = P1SEGADDR(net2272_port_base(dev, EP_DATA));
		unsigned long flags;

		if ((unsigned long)buf & (L1_CACHE_BYTES - 1))
			goto slow_mode;

		BUG_ON(L1_CACHE_BYTES != 32);
		BUG_ON((unsigned long)fifo & (L1_CACHE_BYTES-1));

		local_irq_save(flags);
		fast_read_packet(fifo, (unsigned long *)buf, len >> L1_CACHE_SHIFT);
		local_irq_restore(flags);

		buf += len & ~(L1_CACHE_BYTES - 1);
		len &= L1_CACHE_BYTES - 1;
	}
slow_mode:
	for (; len > 1; len -= 2) {
		*(u16 *)buf = net2272_read_data(dev);
		buf += sizeof (u16);
	}
	if (len > 0)
		*buf++ = net2272_read_data(dev);
}

static void read_packet(struct net2272_ep *ep, u8 *buf, int avail)
{
	struct net2272 *dev = ep->dev;
	...
	if (avail == 0) {
		...
		net2272_read_data(dev);
		return;
	}

	do_read_packet(dev, buf, avail);
...
top:
	do {
		count = net2272_ep_read (ep, EP_AVAIL0) |
			(net2272_ep_read (ep, EP_AVAIL1) << 8);
		...
		buf = req->req.buf + req->req.actual;
		read_packet (ep, buf, count);
		req->req.actual += count;

		/* completion */
		if (unlikely (cleanup || count < ep->ep.maxpacket ||
			...
			if (unlikely (ep->num == 0)) {
				...
				net2272_ep_write (ep, EP_TRANSFER0, 0xff);
			}
...
static int net2272_dequeue (struct usb_ep *_ep, struct usb_request *_req)
{
	...
	unsigned long			flags;
	int				stopped;
	int				rv = -EINVAL; /* this fucking ugly tabs... */
...
static int do_net2272_probe(struct net2272 *dev)
{   // Quick test to see if CPU can communicate properly with the NET2272
	...
	/*
	 * Start probing by _reading_ the scratch reg which has a
	 * direct access.
	 */
...
static int net2272_present(struct net2272 *dev)
{
	...
	for (shift = 0; shift < 20; shift++) {
		dev->shift_addr = shift;
		if (do_net2272_probe(dev) == 0)
			return 0;
	}
	dev->shift_addr = 0;
	return -ENODEV;
...
static void net2272_rdk_remove (struct pci_dev *pdev)
{
    ...
    {
        INFO (dev, "unmapping fpga, %08X\n", (unsigned int) dev->fpga_base_addr);
...
#endif
	if (request_irq (pdev->irq, net2272_irq, IRQF_SHARED, driver_name, dev) != 0)
	{
...
{
	struct net2272 *net2272 = platform_get_drvdata(pdev);
	...
	free_irq(irq, net2272);

	device_remove_file (&pdev->dev, &dev_attr_registers);
	device_unregister(&net2272->gadget.dev);;

	/* clean up resources allocated during probe() */
	iounmap (net2272->base_addr);

	rsrc = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	release_mem_region (rsrc->start, resource_len(rsrc));
...
static int net2272_probe(struct platform_device *pdev)
{
	struct net2272 *net2272;
	struct resource *rsrc;
	void __iomem *regs;
	int irq;
	int retval;
	...
	retval = -EBUSY;
	if (!request_mem_region(rsrc->start, resource_len(rsrc), "net2272"))
		goto err0;

	retval = -ENOMEM;
	regs = ioremap(rsrc->start, resource_len(rsrc));
	if (regs == NULL)
		goto err1;

	if (use_dma) {
		printk("DMA mode is not supported: PLX's joke - disabled\n");
		use_dma = 0;
	}

	/* alloc, and start init */
	net2272 = kzalloc(sizeof *net2272, GFP_KERNEL);
	if (net2272 == NULL)
		goto err2;

	spin_lock_init (&net2272->lock);
	net2272->pdev = pdev;
	net2272->gadget.ops = &net2272_ops;
	net2272->gadget.is_dualspeed = 1;

	/* the "gadget" abstracts/virtualizes the controller */
	device_initialize(&net2272->gadget.dev);
	strcpy (net2272->gadget.dev.bus_id, "gadget");
	net2272->gadget.dev.parent = &pdev->dev;
	net2272->gadget.dev.dma_mask = pdev->dev.dma_mask;
	net2272->gadget.dev.release = gadget_release;
	net2272->gadget.name = driver_name;
	net2272->base_addr = regs;
	net2272->shift_addr = 0;

	net2272->enabled = 1;

	net2272->indexed_threshold = 1 << 5;
	net2272->dma_eot_polarity = 0;
	net2272->dma_dack_polarity = 0;
	net2272->dma_dreq_polarity = 0;
	net2272->dma_busy = 0;

	// See if there..., can remove this test for production code
	if (net2272_present(net2272)) {
		WARN(net2272, "2272 not found!\n");
		retval = -ENODEV;
		goto err3;
	}
	usb_reset (net2272);
	...
	if (request_irq (irq, net2272_irq, 0, driver_name, net2272) != 0) {
		ERROR(net2272, "request interrupt %d failed\n", irq);
		retval = -EBUSY;
		goto err3;
	}

	net2272->chiprev = net2272_read (net2272, CHIPREV_2272);

	/* done */
	INFO (net2272, "%s version: %s\n", driver_desc, DRIVER_VERSION);
	INFO (net2272, "irq %d, mapped mem %p, chip rev %04x\n", irq, regs, net2272->chiprev);
	INFO (net2272, "running in 16-bit, %s byte swap local bus mode\n",
		net2272_read (net2272, LOCCTL) & (1 << BYTE_SWAP) ? "" : "no");
	INFO (net2272, "fifo access method: %s\n",
		net2272->shift_addr > L1_CACHE_SHIFT ? "cache" : "normal");

	retval = device_add (&net2272->gadget.dev);
	if (retval < 0)
		goto err4;
	...
	platform_set_drvdata(pdev, net2272);
	the_controller = net2272;
	return 0;
err5:
	device_unregister(&net2272->gadget.dev);
err4:
