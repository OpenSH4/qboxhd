...
void __init stx7100_configure_usb(void)
{
...
#if !defined CONFIG_HW_WYBOX && !defined CONFIG_SH_QBOXHD_1_0 \
	&& !defined CONFIG_SH_QBOXHD_MINI_1_0
	 ...
	 */
	pin = stpio_request_pin(5,7, "USBPWR", STPIO_IN);
	stpio_set_pin(pin, 1);
#endif
...
void __init stx7100_configure_pata(int bank, int pc_mode, int irq)
{
	...
	if (irq < 0)
		pata_resources[2].flags = IORESOURCE_UNSET;
	else {
...
#if defined CONFIG_SH_QBOXHD_1_0 || defined CONFIG_SH_QBOXHD_MINI_1_0
	printk("Selecting speed to %dMbps\n", (speed == SPEED_100) ? 100 : 10);
#endif /* CONFIG_SH_QBOXHD_1_0 */
...
/* QBoxHD doesn't use this GPIO for the stmmac. It's used for the LNB voltage in i2c-2 */
/* QboxHD_mini uses the internal clk for LAN, so it is requested */
#if !defined CONFIG_HW_WYBOX && !defined CONFIG_SH_QBOXHD_1_0
	/* Remove the PHY clk */
...
void __init stx7100_configure_lirc(lirc_scd_t *scd, unsigned *pios_on,
                                   int npios)
{
