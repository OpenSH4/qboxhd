...
void wymdbox_pio_init(void)
{
	 ...
	 * PIO1_4 = LED2: red
	 * PIO1_5 = LED3: green */
	/* Turn LED1 on, LED2 and LED3 off */
	pio_ptr = stpio_request_set_pin(1, 3, "LED1", STPIO_OUT, 1);
	stpio_free_pin(pio_ptr);
	pio_ptr = stpio_request_set_pin(1, 4, "LED2", STPIO_OUT, 0);
	stpio_free_pin(pio_ptr);
	 ...
	 * PIO1_7 = K8P6415UQB:RYnBY (NOR Flash Ready/Busy output)
	 * PIO2_6 = AT24C08AN:WP (EEPROM Write Protect)
	 * PIO3_6 = K8P6415UQB:nWP (NOR Flash Write Protect)
	 ...
	 * PIO3_5 on MDBOXA_V0100 = CY7C67200:nSSI (USB OTG Host/Peripheral Controller chip select): active low, no external pull-up
	 * PIO3_5 on MDBOXB_V0100 = not connected, external pull-down
	 * PIO3_5 on MDBOXB_V0200 = Factory Reset Button (active low, not debounced) */
	/* The STM Synchronous Serial Controller driver configures
	...
	/** USB **/
	/* PIO2_2 on MDBOXA_V0100 = CY7C67200:nRESET (USB OTG Host/Peripheral Controller)
	 * PIO2_2 on MDBOXB_V0100 = not connected, external pull-down
	 * PIO2_7 = USB2514:nRESET
	 * PIO4_4 on MDBOXA_V0100 = not connected, external pull-down
	 * PIO4_4 on MDBOXB_V0100 = NET2272:nRESET (USB Peripheral Controller) */
	...
	stpio_free_pin(pio_ptr);
	/* Reset the Cypress USB Host/Peripheral Controller
	 * assert nRESET (low) for at least 1.4usec */
	pio_ptr = stpio_request_set_pin(2, 2, "USB_HOST_RESET", STPIO_OUT, 0);
	udelay(2);
	stpio_set_pin(pio_ptr, 1);
	stpio_free_pin(pio_ptr);
	/* Reset the USB hub
	...
	/** DVB-T receivers **/
	/* PIO2_4 = STV0362:nRESET (DVB-T receiver 1)
	 * PIO2_5 = STV0362:nRESET (DVB-T receiver 2) */
	 ...
	 * PIO3_5 on MDBOXB_V0100 = not connected, external pull-down
	 * PIO3_5 on MDBOXB_V0200 = Factory Reset Button (active low, not debounced) */
	/* hence configure PIO3_5 as a general purpose INPUT with weak pull-up */
	pio_ptr = stpio_request_pin(3, 5, "RESET_BUTTON", STPIO_BIDIR_Z1);
	stpio_free_pin(pio_ptr);
}
