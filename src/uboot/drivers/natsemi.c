...
#define io_bus_to_phys(a)	pci_io_to_phys((pci_dev_t)dev->priv, a)
#define io_phys_to_bus(a)	pci_phys_to_io((pci_dev_t)dev->priv, a)
...
natsemi_initialize(bd_t * bis)
{
	...
	while (1) {
		...
		dev->iobase = bus_to_phys(iobase); /* bus_to_phys(iobase); */
#ifdef NATSEMI_DEBUG
