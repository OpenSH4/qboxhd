...
//#define ADVERT_HD_ONLY
//#define CONFIG_STMAC_FLOWCTRL
...
//QBOXHD
#define MASK_ID_LAN8700			0xFFFFFFF0	// It is read like id

#define	KSZ8995XA_HI_ID			0x0022
#define	KSZ8995XA_LO_ID			0x1450
...
static unsigned int stmac_phy_check_speed (int phy_addr)
{
	...
	printf ("STMAC: Port %d :",phy_addr);

	/* Check link status.  If 0, default to 100 Mbps. */
	if ((status & BMSR_LSTATUS) == 0) {
		printf ("*Warning* no link detected\n");
		//if (phy_addr == 1)
		if (phy_addr == PHY_ADDRESS_CHIP)
			IsLinked=0x00;
		return 1;
	} else {
		int capa = stmac_mii_read(phy_addr, MII_ADVERTISE);
		int negotiated_lpa = stmac_mii_read (phy_addr, MII_LPA);
		int negotiated = capa & negotiated_lpa;

		if (negotiated & LPA_100FULL) {
			printf ("100Mbs fd link detected\n");
			full_duplex = 1;
			speed = 100;
			//if (phy_addr == 1)
			if (phy_addr == PHY_ADDRESS_CHIP)
			...
			printf ("100Mbs hd link detected\n");
			full_duplex = 0;
			speed = 100;
			//if (phy_addr == 1)
			if (phy_addr == PHY_ADDRESS_CHIP)
		...
		//} else {
			printf ("10Mbs hd link detected\n");
			full_duplex = 0;
			speed = 10;
			//if (phy_addr == 1)
			if (phy_addr == PHY_ADDRESS_CHIP)
				IsLinked=0x01;
		} else {						// forced link speed and duplex
			capa = stmac_mii_read(phy_addr, MII_BMCR);
			if (capa & BMCR_SPEED100) {
				if (capa & BMCR_FULLDPLX)
					printf ("100Mbs fd link detected\n");
				else
					printf ("100Mbs hd link detected\n");
			} else {
				if (capa & BMCR_FULLDPLX)
                    printf ("10Mbs fdlink detected\n");
				else
					printf ("10Mbs hd link detected\n");
			}
...
/* In qboxhd the speed mac is forced from 'stmac_phy_init' */
...
/*		if ((id1 == LAN8700_PHY_HI_ID) &&
		...
		if ((id1 == LAN8700_PHY_HI_ID) &&
		    ((id2&MASK_ID_LAN8700) == (LAN8700_PHY_LO_ID&MASK_ID_LAN8700))) {
			stmac_phy_id = id1;
			printf ("STMAC: SMSC LAN8700 found\n");
		}
#endif	/* CONFIG_STMAC_LAN8700 */

		/* For KSZ8995XA switch */
		if ((id1 == KSZ8995XA_HI_ID) &&
		    (id2 == KSZ8995XA_LO_ID)) {
			stmac_phy_id=KSZ8995XA_HI_ID;
			printf ("STMAC:KSZ8995XA\n");
			printf ("phy address:0x%04X\n",phyaddr);
		}

		//So if there is some Phy but it is not known, try default phy
		if( (stmac_phy_id==0) && ((id1!=0xFFFF)||(id2!=0xFFFF)) )
		{		
			printf ("## WARNING ## Unknown PHY, using default SMSC LAN8700\n");
			printf("ID1=0x%08X\n",id1);
			printf("ID2=0x%08X\n",id2);
			stmac_phy_id=LAN8700_PHY_HI_ID;
		}
		if (stmac_phy_id)
			return phyaddr;
	}
...
//QBOXHD
//	stmac_phy_reset (0);
stmac_phy_reset (PHY_ADDRESS_CHIP);

	eth_phy_addr = stmac_phy_get_addr ();

  printf("Eth_phy_addr: %x\n",eth_phy_addr);
...
/*
	if (value != eth_phy_addr) {
		...
		//	value,
		//	eth_phy_addr);
		printf ("PHY addr mismatch with hardware (hw %d != %d)\n",
			value,
			eth_phy_addr);
	}
*/
...
static int stmac_phy_init (void)
{
	uint advertised_caps, value, tmp_mii;
	int full_duplex = 0;
	int speed = 0;
	int j;
//QBOXHD
...
*/
	...
	/* *********************************************************** Port phy_addr 1 (external LAN): set to autoneg*/
	/* Read the ANE Advertisement register */
	advertised_caps = stmac_mii_read (eth_phy_addr, MII_ADVERTISE);
...
//		advertised_caps |= ADVERTISE_100BASE4;
	...
	/* Update our Auto-Neg Advertisement Register */
	stmac_mii_write (eth_phy_addr, MII_ADVERTISE, advertised_caps);
	//printf("Port 1 MII advertised caps: 0x%04X \n",advertised_caps);
//#endif
	stmac_phy_negotiate (eth_phy_addr);

/*      tmp_mii=stmac_mii_read (1, MII_BMCR);
        printf("Port 1 MII config: 0x%04X \n",tmp_mii);
        tmp_mii &= (~BMCR_ANENABLE);
        tmp_mii |= (BMCR_SPEED100);
        tmp_mii &= (~BMCR_FULLDPLX);
        stmac_mii_write (1, MII_BMCR, tmp_mii);
        udelay(1000);
        tmp_mii=stmac_mii_read (1, MII_BMCR);
        printf("Port 1 MII config: 0x%04X \n",tmp_mii);
*/
	...
	/* *********************************************************** Port phy_addr 2 & 3 (unused): set to power down */
	for (j=2;j<4;j++)
	{
		tmp_mii=stmac_mii_read (j, MII_BMCR);
		tmp_mii|=BMCR_PDOWN;
		stmac_mii_write (j, MII_BMCR, tmp_mii);
	}

	/* Set port 4, 5 and internal mac to 100FD */
	/* *********************************************************** Port phy_addr 4 & 5 (connect STi710x to switch): force speed and duplex */
	for (j=4;j<6;j++)
	{
		tmp_mii=stmac_mii_read (j, MII_BMCR);
		tmp_mii &=(~BMCR_ANENABLE);			// disable AN
		tmp_mii |= BMCR_SPEED100;			// force speed 100
		tmp_mii |= BMCR_FULLDPLX;			// force FD
		//tmp_mii &=(~BMCR_FULLDPLX);			// force HD
		stmac_mii_write (j, MII_BMCR, tmp_mii);
	}

	/* *********************************************************** MII connection between STi710x and switch */

	/* There is a switch net so it may be set to 100 FD */
//	full_duplex=0;
	full_duplex=1;
	speed=100;
	stmac_set_mac_mii_cap (full_duplex, speed);
...
//QBOXHD
//The MII registers are inverted 
...
static int stmac_reset_eth (bd_t * bd)
{
	...
	unsigned int status;
	/* Read Status register */
	status = stmac_mii_read (eth_phy_addr, MII_BMSR);
	/* Check link status.  If 0, default to 100 Mbps. */
	if( (status!=0xFFFF)&& ((status & BMSR_LSTATUS) != 0) )//link already set 
	{
		printf("LINK\n");
		return (0);
	}
...
{
	PRINTK ("%s\n", __FUNCTION__);
//QBOXHD: dummmy function, create 'eth_halt_special'
//	char *str;
//	str = getenv("stdin");
//	if(strcmp(str,"nc")==0)	//net console activate, so no re-init eth
//		return;
...
int prova_stmac_mii_read (int phy_addr, int reg)
{
	return stmac_mii_read (phy_addr, reg);
}
