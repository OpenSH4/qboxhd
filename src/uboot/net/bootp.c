...
/* TIMEOUT_COUNT is used only in bootp (for qbox ionly in dhcp) */
...
static void Dhcp_Only_IPs_Handler(uchar * pkt, unsigned dest, unsigned src, unsigned len);
/* For Debug */
#if 0
...
/* In this version of u-boot there is CMD_DHCP. This flags disable 'bootp' and enable 'dhcp'. */
/* So BootpTimeout is changed for DHCP_Only_IPs */ 
#if 0
static void
BootpTimeout(void)
{
...
BootpTimeout(void)
{
	if(BootpTry >= TIMEOUT_COUNT)
	{
		printf("\nDHCP failed....Use the static IP in environment\n");
		NetState = NETLOOP_SUCCESS;
	}
	else
	{
...
//	printf("BOOTP broadcast %d\n", ++BootpTry);
	...
	if(NetServerIP==0)
		NetSetHandler(Dhcp_Only_IPs_Handler);
	else
...
//Duolabs
/*
 *	Handle DHCP received packets only to obtain the Ips.
 */
static void
Dhcp_Only_IPs_Handler(uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	Bootp_t *bp = (Bootp_t *)pkt;

	debug ("Dhcp_Only_Ips_Handler: got packet: (src=%d, dst=%d, len=%d) state: %d\n",
		src, dest, len, dhcp_state);

	if (BootpCheckPkt(pkt, dest, src, len)) /* Filter out pkts we don't want */
		return;

	debug ("Dhcp_Only_Ips_Handler: got DHCP packet: (src=%d, dst=%d, len=%d) state: %d\n",
		src, dest, len, dhcp_state);
