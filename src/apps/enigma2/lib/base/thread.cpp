...
int eThread::runAsync(int prio, int policy)
{
	...
	eDebug("eThread::runAsync() - the_thread=%d",the_thread);
	pthread_attr_destroy(&attr);
	return 0;
}
...
int eThread::sendSignal(int sig)
{
	if (m_alive)
	{
		eDebug("eThread::sendSignal(): send signal (%d) to thread (%d)",sig,the_thread);
...
void eThread::kill(bool sendcancel)
{
	...
	eDebug("eThread::kill() - waiting for thread join ...");
	eDebug("thread joined %d", pthread_join(the_thread, 0));

	the_thread = 0;
	eDebug("eThread::kill() - DONE.");
}
