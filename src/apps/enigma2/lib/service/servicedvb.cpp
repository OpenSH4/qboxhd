...
#ifdef QBOXHD
	m_tune_timer = eTimer::create(eApp);
	CONNECT(m_tune_timer->timeout, eDVBServicePlay::startTune);
#endif
}
...
{
	eDebug("eDVBServicePlay::serviceEvent() - called");
	...
	switch (event)
	{
		...
		{
			eDebug("eDVBServicePlay::serviceEvent() - eDVBServicePMTHandler::eventTuned");
		...
		{
			eDebug("eDVBServicePlay::serviceEvent() - eDVBServicePMTHandler::eventNewProgramInfo");
		...
		case eDVBServicePMTHandler::eventEOF:
			eDebug("eDVBServicePlay::serviceEvent() - eDVBServicePMTHandler::eventEOF");
			m_event((iPlayableService*)this, evEOF);
			break;
		case eDVBServicePMTHandler::eventSOF:
			eDebug("eDVBServicePlay::serviceEvent() - eDVBServicePMTHandler::eventSOF");
...
void eDVBServicePlay::serviceEventTimeshift(int event)
{
	...
	{
	case eDVBServicePMTHandler::eventNewProgramInfo:
// 		eDebug("eDVBServicePlay::serviceEventTimeshift(eventNewProgramInfo) m_timeshift_enabled=%d - m_timeshift_active=%d, m_timeshift_changed=%d",
// 				m_timeshift_enabled, m_timeshift_active,m_timeshift_changed);
		if (m_timeshift_active)
			updateDecoder();
		break;
	case eDVBServicePMTHandler::eventSOF:
...
{
	eDebug("eDVBServicePlay::start() - called.");
	...
	if (m_is_pvr)
	{
...
#ifdef QBOXHD
		///try to anticipate hide of media list at start play.
		...
		m_event(this, evStart);
#endif ///QBOXHD
		eDebug("eDVBServicePlay::start() - m_is_pvr==1 - DONE.");
		return 0;
	}
	else
		m_event(this, evStart);
	...
	m_tune_timer->stop();
	m_tune_timer->start(300, 1);

	return 0;
}

void eDVBServicePlay::startTune()
{
	int r;
		/* in pvr mode, we only want to use one demux. in tv mode, we're using
		   two (one for decoding, one for data source), as we must be prepared
...
RESULT eDVBServicePlay::pause(ePtr<iPauseableService> &ptr)
{
		/* note: we check for timeshift to be enabled,
		   not necessary active. if you pause when timeshift
...
RESULT eDVBServicePlay::setFastForward_internal(int ratio)
{
	...
	{
		eDebug("eDVBServicePlay::setFastForward_internal(): setting cue skipmode to %d", skipmode);
		if (m_cue)
...
///git2svn-syncpoint-experimental-1-g2013076
#define E2_PATCH_AM_26_11_2009

int eDVBServicePlay::selectAudioStream(int i)
{
	eDVBServicePMTHandler::program program;
	eDVBServicePMTHandler &h = m_timeshift_active ? m_service_handler_timeshift : m_service_handler;
#ifdef E2_PATCH_AM_26_11_2009
	pts_t position = -1;
#endif ///E2_PATCH_AM_26_11_2009
	...
	if (!(m_is_pvr || m_timeshift_active || !m_is_primary))
	{
		...
		{
			eDebug("VideoStream is empty, so the rdsPid filter is started\n");
	#endif
		...
		{
			eDebug("VideoStream isn't empty, so the rdsPid filter isn't started\n");
		}
	#endif
	}
...
// eDebug("eDVBServicePlay::activateTimeshift ... m_timeshift_active=%d",m_timeshift_active);
...
void eDVBServicePlay::updateDecoder()
{
	...
	{
		eDebugNoNewLine("eDVBServicePlay::updateDecoder(): have %d video stream(s)", program.videoStreams.size());
		if (!program.videoStreams.empty())
		{
			eDebugNoNewLine(" (");
			for (std::vector<eDVBServicePMTHandler::videoStream>::const_iterator
...
#ifdef QBOXHD
    // If video stream is empty and audio stream is not empty, we have a radio service
    if (!program.videoStreams.size() && program.audioStreams.size())
    {
        radio_service = 1;
    }
    else
