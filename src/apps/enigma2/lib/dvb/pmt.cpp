...
{
	int state;
	eDebug("eDVBServicePMTHandler::channelStateChanged() - called");

	channel->getState(state);

// 	eDebug("eDVBServicePMTHandler::channelStateChanged() - m_last_channel_state=%d,state=%d,m_demux=<%p>",m_last_channel_state,state,(eDVBChannel *)m_demux);
	eDebug("eDVBServicePMTHandler::channelStateChanged() - m_last_channel_state=%d,state=%d",m_last_channel_state,state);
		...
		&& (state == iDVBChannel::state_ok) && (!m_demux))
	{
		if (m_channel)
			if (m_channel->getDemux(m_demux, (!m_use_decode_demux) ? 0 : iDVBChannel::capDecode))
				eDebug("Allocating a %s-decoding demux for now tuned-in channel failed.", m_use_decode_demux ? "" : "non-");

		serviceEvent(eventTuned);
		if (m_demux)
		{
			eDebug("eDVBServicePMTHandler::channelStateChanged() - ok ... now we start!!!");
...
eDebug("eDVBServicePMTHandler::channelStateChanged() - DONE.");
}
...
#ifdef QBOXHD
	{
		eDebug("eDVBServicePMTHandler::PMTready(error=%d) m_pvr_channel=%s",error,m_pvr_channel?"YES":"NO");
		if(!m_pvr_channel)
			serviceEvent(eventNoPMT);
		else
			eDebug("---------------- \n\t\t\teDVBServicePMTHandler::PMTready(error=%d) EVENT DROPPED\n\n",error);
	}
#else
		serviceEvent(eventNoPMT);
#endif //QBOXHD
...
#ifdef QBOXHD
				m_ca_servicePtr->buildCAPMT(ptr,m_demux->getAdapter());
#else
...
void eDVBServicePMTHandler::PATready(int)
{
	...
	if (!m_PAT.getCurrent(ptr))
	{
		...
		if (pmtpid == -1)
#ifdef QBOXHD
            eDebug("No PAT entry: Service not found! (SID not found in PAT)");
#else
...
//													s.language_code.c_str(), s.pid, s.teletext_page_number, s.teletext_magazine_number);
					...
					{
						audio.pid = (*es)->getPid();
// 						eDebug("%s(): found PID %04x  <<<<<<<<<zzzzzzzzzzzzzzzzzzzzzzzzz ", __func__, audio.pid);
	...
	else if ( m_service && !m_service->cacheEmpty() )
	{
		...
		if ( cached_apid_ac3 != -1 )
		{
			...
			++cnt;
// 			eDebug("%s(): cnt=%d audio AC3 PID %04x detected", __func__, cnt, s.pid);
		}
		if ( cached_apid_mpeg != -1 )
		{
			...
			program.audioStreams.push_back(s);
			++cnt;
// 			eDebug("%s(): cnt=%d audio MPEG PID %04x detected", __func__, cnt, s.pid);
		}
		if ( cached_pcrpid != -1 )
		{
			++cnt;
...
{
	RESULT res=0;
eDebug("eDVBServicePMTHandler::tune (use_decode_demux=%d, simulate=%s, service=<%p>)\n",use_decode_demux,simulate?"TRUE":"FALSE",service);
	...
	if (ref.path.empty())
	{
		...
		if (!res && !simulate) {
			...
			m_channel->getFrontend(fe_tmp);
			eDVBFrontend *fe2 = (eDVBFrontend*) &(*fe_tmp);
			dvbid_tmp = fe2->getDVBID();

			eDelayerManager* m_delayer_manager = eDelayerManager::getInstance();
			eDelayerAdapter* m_delayer_adapter = 0;

			m_delayer_adapter = m_delayer_manager->getDelayerAdapter( dvbid_tmp );
			if (m_delayer_adapter) {
				if ( m_delayer_adapter->isActive() ) {
					eDebug("[DELAYER] already active. Setting time delay to %d\n", m_delayer_adapter->getDelayTime());
					m_delayer_adapter->setDelayTime(m_delayer_adapter->getDelayTime());
				}
				else
					eDebug("[DELAYER] not active.\n");
			}
#endif
#endif
...
pthread_mutex_t eDVBCAService::send_lock = PTHREAD_MUTEX_INITIALIZER;
unsigned char eDVBCAService::m_toggle_frontend = 0xFF;
...
int eDVBCAService::comunicate_generic_info(const char *buf)
{
	int                 sock, res=-1;
...
#ifdef QBOXHD
	m_timeoutTimer = eTimer::create(eApp);
	CONNECT(m_timeoutTimer->timeout, eDVBCAService::timeoutCAPMT);
#endif
	Connect();
}
...
#ifdef QBOXHD
	if ( eDVBCAService::m_toggle_frontend == m_adapterId )
	    eDVBCAService::m_toggle_frontend = 0xFF;
#endif	
	::close(m_sock);
}
...
#ifdef QBOXHD
void eDVBCAService::buildCAPMT(eTable<ProgramMapSection> *ptr, unsigned char adapterId)
#else
...
//			eDebug("append");
		...
		tmp[4] = adapterId  & 0xFF; // add adapter number
#else
		tmp[0] = 0x82; // demux
		tmp[1] = 0x02;
		tmp[2] = demux_mask;	// descramble bitmask
...
void eDVBCAService::timeoutCAPMT()
{
      pthread_mutex_lock(&eDVBCAService::send_lock);
      eDVBCAService::m_toggle_frontend = 0xFF;
      pthread_mutex_unlock(&eDVBCAService::send_lock);
}
...
void eDVBCAService::sendCAPMT()
{
	...
	/* eDVBCAService::m_toggle_frontend = 0xFF 	  ( Ready for send data to frontend.socket and camd.socket )*/
	/* eDVBCAService::m_toggle_frontend = m_adapterId ( frontend already sended for this adapter in frontend.socket. 
							    Send only pmt for this adapter to camd.socket )*/
	if ( (eDVBCAService::m_toggle_frontend != 0xFF) && (eDVBCAService::m_toggle_frontend != m_adapterId ) ) {
	    pthread_mutex_unlock(&eDVBCAService::send_lock);
	    m_retryTimer->start(100,true);
	    return;
	}

	if (eDVBCAService::m_toggle_frontend == 0xFF) {
	    /* frontend.socket and frontend.tmp */
	    /* Get information about orbital_position and frequency of this channel */
	    m_frequency = 0;
	    ...
	    if (data)
	    {
		    data->m_channel->getCurrentFrontendParameters(m_ch_current);
		    ...
		    if (type == iDVBFrontend::feSatellite)
		    {
			    ...
			    m_frequency = parm.frequency;
			    m_orbital_position = parm.orbital_position;
		    }
	    }
	    sprintf( data_str, "%d %08x %04x", m_adapterId, m_frequency, m_orbital_position );
	    
	    if (comunicate_generic_info(data_str) == 0) {
//		eDebug("[eDVBCAService]->frontend.socket: m_adapterId=%d send %s", m_adapterId, data_str);
		eDVBCAService::m_toggle_frontend = m_adapterId;
	    }
	}

	/* Write which fronted are you using in frontend.tmp */
	while(1) {
		FILE *frontendfd=0;
		char data_hex[50];

		frontendfd = fopen("/tmp/frontend.tmp", "w+");
		if (!frontendfd) {
			eDebug("[eDVBCAService] open /tmp/frontend.tmp error");
			break;
		}

		sprintf(data_hex, "%02x %08x %04x", m_adapterId, m_frequency, m_orbital_position);
		fwrite( data_hex, 1, strlen(data_hex), frontendfd );
	...
	while(1) {
		...
		for (i=0; i<wp; i++) {
			sprintf(m_pmt_hex, "%02X", m_capmt[i]);
			strcat(pmt_hex, m_pmt_hex);
		}

		fwrite(pmt_hex, 1, strlen(pmt_hex), pmtfd);
...
#ifdef QBOXHD
//			eDebug("[eDVBCAService]->camd.socket: m_adapterId=%d send %d bytes: %s", m_adapterId, wp, tmp);
#else
//			eDebug("[eDVBCAService] send %d bytes: %s", wp,tmp);
#endif			
		}
	...
	else
	{
		switch(m_sendstate)
		{
			case 0xFFFFFFFF:
				++m_sendstate;
#ifdef QBOXHD
				if ( ( eDVBCAService::m_toggle_frontend != 0xFF) && (!m_timeoutTimer->isActive()) )
