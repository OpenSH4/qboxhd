	...
	switch(type)
	{
		...
		case signalQualitydB: /* this will move into the driver */
		{
			...
			if (type == signalQuality)
			{
                ...
                if (!strcmp(m_description, "AVL2108")) {
                    double d_snr = (double)snr / 1000.0;
                    d_snr = pow(10, d_snr);
                    snr = 100 - (100 / d_snr);
                    snr *= 65535 / 100;
//                     eDebug("-----> SNR: %d, d_snr: %lf\n", snr, d_snr);
                }
                if (!strcmp(m_description, "TDA10048"))
				{
                    snr *= 65535 / 255;
				}
...
#ifdef QBOXHD
#ifdef QBOXHD_MINI
		PutToDict(dest, "adapter_number", m_dvbid);
#endif
#endif
