...
public:
	 ...
	 */
#ifdef QBOXHD
	void setRCIdentifier(std::string rc_id);
	const std::string getRCIdentifier() const { return rc_id; }
#endif
...
/* DirectFB input driver */
class eDirectFBInputEventDriver: public eRCDriver
{
protected:
	int handle;
	ePtr<eSocketNotifier> sn;
...
public:
	std::string getDeviceName();
	eDirectFBInputEventDriver();
	~eDirectFBInputEventDriver();

	ePtr<gFBDC>            m_fb;
	IDirectFB              *dfb;
	IDirectFBEventBuffer   *event_buff;
    	DFBException           *ex;
};
