	...
	def __init__(self, session, fullname, name, ref):
		...
		if currPlay is not None:
			if QBOXHD:
				...
				tlist.append(ServiceInfoListEntry("Title: ", stitle))
				tlist.append(ServiceInfoListEntry("Artist: ", currPlay.info().getInfoString(iServiceInformation.sArtist)))
				tlist.append(ServiceInfoListEntry("Album: ", currPlay.info().getInfoString(iServiceInformation.sAlbum)))
				tlist.append(ServiceInfoListEntry("Genre: ", currPlay.info().getInfoString(iServiceInformation.sGenre)))
				tlist.append(ServiceInfoListEntry("Year: ", currPlay.info().getInfoString(iServiceInformation.sTimeCreate)))
				tlist.append(ServiceInfoListEntry("Comment: ", currPlay.info().getInfoString(iServiceInformation.sTagComment)))
...
class MC_FavoriteFolders(Screen):
	def __init__(self, session):
		...
		for i in range(0, config.plugins.mc_favorites.foldercount.value):
			...
			if QBOXHD:
				self.list.append((cfg.name.value, cfg.basedir.value, "50"))
			else:
	...
	def FavoriteFolderEdit(self):
		...
		if selection is None:
			...
			if QBOXHD:
				self.session.openWithCallback(self.conditionalEdit, MC_FavoriteFolderEdit, self["favoritelist"].getIndex())
			else:
	...
	def refresh_mc_favorites_folders_from_list(self):
		#delete all mc_favorites in settings
		...
		for entry in self.list:
			...
			config.plugins.mc_favorites.folders.append(ConfigSubsection())
			config.plugins.mc_favorites.folders[idx].name = ConfigText("", False)
			config.plugins.mc_favorites.folders[idx].name.value = str(entry[0])
			config.plugins.mc_favorites.folders[idx].basedir = ConfigText("/", False)
			config.plugins.mc_favorites.folders[idx].basedir.value = str(entry[1])
			idx+=1
	...
	def deleteConfirm(self, result):
		if result:
			if QBOXHD:
				...
				if selection is None:
					return
				self.list.remove( self["favoritelist"].getCurrent() )
				if QBOXHD:
					self.refresh_mc_favorites_folders_from_list()
				configfile.save()
			else:
				selection = self["favoritelist"].getCurrent()
				id = int(selection[1])
				print "deleteConfirm ", str(id)
	...
	def conditionalNew(self,added):
		...
		if QBOXHD:
			self.list.insert(id, (str(config.plugins.mc_favorites.folders[id].name.value), str(config.plugins.mc_favorites.folders[id].basedir.value), "50"))
		else:
