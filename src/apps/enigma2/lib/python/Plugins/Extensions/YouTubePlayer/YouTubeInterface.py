...
class YouTubeEntry():
	...
	def getYouTubeId(self):
		...
		if self.entry.media.player:
			...
			line = self.entry.media.player.url
			ret = line[line.index('v=')+2 :line.index('&feature=')]
			#print ret
	...
	def getVideoUrl(self, fmt):
		...
		try:
			...
			while not None:
				...
				m = re.search("'SWF_ARGS':.*\"t\":\s\"(?P<id>\S+)\"", data)
				if m:
					#print m
					break
			response.close
			if m:
				...
				mrl = "http://www.youtube.com/get_video?video_id=" + quote(self.getYouTubeId()) + "&t=" + m.group('id') + "&fmt=" + fmt
	...
	def login(self, user):
		...
		if user is not None:
			try:
				...
				self.ytService.developer_key = "AI39si5oyt3NGsYeEc2B0CUkVe5ornmN1tObNH2pRgh9MrKqUZ_3yp0bF-CBbt1gGowB6wAwUz7ZrenYi1P7jlvGCANQvzaBYA"
				#"AI39si7t0WNyg_tvjBPdRIvBfaUA_XrTY1LNzfjLgCn8A_m92YKtWTcR_auEmI5gKGitJb4SskrjxJSmRc3yhQ4YlHTBAzPSig"
