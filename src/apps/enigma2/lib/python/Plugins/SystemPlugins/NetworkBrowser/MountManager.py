...
class AutoMountManager(Screen):
	...
	def updateList(self):
		...
		if QBOXHD:
			self.list.append((_("Add new network mount point"),"add", _("Add a new NFS or CIFS mount point to your QBoxHD."), okpng ))
			self.list.append((_("Mountpoints management"),"view", _("View, edit or delete mountpoints on your QBoxHD."), okpng ))
			self.list.append((_("User management"),"user", _("View, edit or delete usernames and passwords for your network."), okpng))
			self.list.append((_("Change hostname"),"hostname", _("Change the hostname of your QBoxHD."), okpng))
		else:
			self.list.append((_("Add new network mount point"),"add", _("Add a new NFS or CIFS mount point to your QBoxHD."), okpng ))
			self.list.append((_("Mountpoints management"),"view", _("View, edit or delete mountpoints on your QBoxHD."), okpng ))
			self.list.append((_("User management"),"user", _("View, edit or delete usernames and passwords for your network."), okpng))
	...
	def hostEdit(self):
		if os_path.exists("/etc/hostname"):
			...
			fp.close()
			self.session.openWithCallback(self.hostnameCallback, VirtualKeyBoard, title = (_("Enter new hostname for your QBoxHD")), text = self.hostname)
