...
class UpdatePluginMenu(Screen):
	...
	def __init__(self, session, args = 0):
		...
		if self.menu == 0:
			self.list.append(("software-update", _("Software update"), _("\nOnline update of your QBoxHD software." ) + self.oktext) )
			#self.list.append(("install-plugins", _("Install extensions"), _("\nInstall new Extensions or Plugins to your dreambox" ) + self.oktext) )
			self.list.append(("software-restore", _("Software restore"), _("\nRestore your QBoxHD with a new firmware." ) + self.oktext))
			self.list.append(("system-backup", _("Backup system settings"), _("\nBackup your QBoxHD settings." ) + self.oktext))
			self.list.append(("system-restore",_("Restore system settings"), _("\nRestore your QBoxHD settings." ) + self.oktext))
		...
		if current:
			...
			if self.menu == 0:
				if (current == "software-update"):
					self.session.openWithCallback(self.runUpgrade, MessageBox, _("Do you want to update your QBoxHD?")+"\n"+_("\nAfter pressing OK, please wait!"))
...
class PacketManager(Screen):
	...
	def runRemoveFinished(self):
		self.session.openWithCallback(self.RemoveReboot, MessageBox, _("Remove finished.") +" "+_("Do you want to reboot your QBoxHD?"), MessageBox.TYPE_YESNO)
	...
	def runUpgradeFinished(self):
		self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your QBoxHD?"), MessageBox.TYPE_YESNO)
...
class PluginManager(Screen, DreamInfoHandler):
	...
	def runExecuteFinished(self):
		self.session.openWithCallback(self.ExecuteReboot, MessageBox, _("Install or remove finished.") +" "+_("Do you want to reboot your QBoxHD?"), MessageBox.TYPE_YESNO)
		...
		if self.cmdlist is not None:
			for entry in self.cmdlist:
				...
				if cmd == 0:
					...
					info = _("QBoxHD software because updates are available.")
...
class PluginDetails(Screen, DreamInfoHandler):
	...
	def runUpgradeFinished(self):
		self.session.openWithCallback(self.UpgradeReboot, MessageBox, _("Installation finished.") +" "+_("Do you want to reboot your QBoxHD?"), MessageBox.TYPE_YESNO)
...
class UpdatePlugin(Screen):
	...
	def __init__(self, session, args = None):
		...
		self.status = Label(_("Upgrading QBoxHD... Please wait"))
	...
	def ipkgCallback(self, event, param):
		if event == IpkgComponent.EVENT_DOWNLOAD:
			...
			if self.updating:
				...
				self.activityTimer.stop()
				self.activityslider.setValue(0)
				error = _("your QBoxHD might be unusable now. Please consult the manual for further assistance before rebooting your QBoxHD.")
				if self.packages == 0:
					error = _("No packages were upgraded yet. So you can check your network and try again.")
				if self.updating:
					error = _("Your QBoxHD isn't connected to the internet properly. Please check it and try again.")
	...
	def exit(self):
		if not self.ipkg.isRunning():
			if self.packages != 0 and self.error == 0:
				self.session.openWithCallback(self.exitAnswer, MessageBox, _("Upgrade finished.") +" "+_("Do you want to reboot your QBoxHD?"))
			else:
				self.close()
