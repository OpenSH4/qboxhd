...
from config import config, ConfigSlider, ConfigSelection, ConfigYesNo, \
	ConfigEnableDisable, ConfigSubsection, ConfigBoolean, ConfigNumber, ConfigOnOff
...
class AVSwitch:
	...
	if QBOXHD:
		#disable/enable the video and audio output
		def disable_scart_hdmi(self, value):
			eAVSwitch.getInstance().disable_scart_hdmi(value)
...
def InitAVSwitch():
	...
	if QBOXHD:
		colorformat_choices = {"cvbs": _("CVBS"), "rgb": _("RGB")}
	else:
	...
	if can_downmix:
		...
		if QBOXHD:	
			config.av.downmix_ac3 = ConfigOnOff(default = True)
		else:
	...
	if can_osd_alpha:
		if QBOXHD:
			config.av.osd_alpha = ConfigSlider(default=255, increment=15, limits=(0,255))
		else:
