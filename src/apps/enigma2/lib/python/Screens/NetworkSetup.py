...
class NetworkAdapterSelection(Screen,HelpableScreen):
	def __init__(self, session):
		...
		self.wlan_errortext = _("No working wireless networkadapter found.\nPlease verify that you have WiFi module and your Network is configured correctly.")
	...
	def NetworkFallback(self):
		...
		if QBOXHD and not QBOXHD_MINI:
			if iNetwork.configuredNetworkAdapters.has_key('eth0') and (iNetwork.check_iNIC_WIFI_Module_active() ):
				self.session.openWithCallback(self.ErrorMessageClosed, MessageBox, self.wlan_errortext, type = MessageBox.TYPE_INFO,timeout = 10)
...
class AdapterSetup(Screen, ConfigListScreen, HelpableScreen):
	...
	def createConfig(self):
		...
		self.nwlist = None
		if QBOXHD:
			self.InterfaceWiFiEntry = None
			self.wepKeyType = None
			self.WiFiconfig =  None
			self.WiFi = None
		else:
			self.wsconfig = None
		...
		if QBOXHD:
			if ( self.iface == "wlan0" ) or ( not QBOXHD_MINI and self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ):
				driver = iNetwork.detectWlanModule(self.iface)

				if driver == 'zd1211rw' and self.iface == 'wlan0':
					from Plugins.SystemPlugins.WirelessLan.Wlan import wpaSupplicant,Wlan
					self.WiFi = wpaSupplicant()
				  
				elif driver == 'rt73' and self.iface == 'wlan0':
					from Plugins.SystemPlugins.WirelessLan.Wlan import USB_WiFi_Module,Wlan
					self.WiFi = USB_WiFi_Module()

				elif ( not QBOXHD_MINI and self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ):
					from Plugins.SystemPlugins.WirelessLan.Wlan import iNIC_MII_WiFi_Module,Wlan
					self.WiFi = iNIC_MII_WiFi_Module()
				  
				self.encryptionlist = []
				self.encryptionlist.append(("WEP 64", _("WEP 64")))
				self.encryptionlist.append(("WEP 128", _("WEP 128")))
				...
				if self.aplist is not None:
					...
					try:
						...
						if self.aps is not None:
							print "[NetworkSetup.py] got Accesspoints!"
							for ap in self.aps:
								a = self.aps[ap]
								if a['active']:
									if a['essid'] == "":
				...
				self.WiFiconfig = self.WiFi.loadConfig()
				self.default = self.essid or self.WiFiconfig['ssid']
				...
				config.plugins.wlan.hiddenessid = NoSave(ConfigText(default = self.WiFiconfig['hiddenessid'], visible_width = 50, fixed_size = False))
				config.plugins.wlan.essidscan = NoSave(ConfigYesNo(default = self.WiFiconfig['ssidscan']))

				if self.WiFiconfig['ssidscan'] is True:
					config.plugins.wlan.essid = NoSave(ConfigSelection(self.nwlist, default = self.default ))
				else:
					config.plugins.wlan.essid = NoSave(ConfigText(default = self.default, visible_width = 50, fixed_size = False))

				config.plugins.wlan.encryption.enabled = NoSave(ConfigYesNo(default = self.WiFiconfig['encryption'] ))
				config.plugins.wlan.encryption.type = NoSave(ConfigSelection(self.encryptionlist, default = self.WiFiconfig['encryption_type'] ))
				config.plugins.wlan.encryption.wepkeytype = NoSave(ConfigSelection(self.weplist, default = self.WiFiconfig['encryption_wepkeytype'] ))
				config.plugins.wlan.encryption.psk = NoSave(ConfigPassword(default = self.WiFiconfig['key'], visible_width = 50, fixed_size = False))
				config.plugins.wlan.encryption.psk.setAsciiChars()

				if config.plugins.wlan.encryption.wepkeytype.value == 'HEX' and ( config.plugins.wlan.encryption.type.value == 'WEP 64' or config.plugins.wlan.encryption.type.value == 'WEP 128' ):
					config.plugins.wlan.encryption.psk.setHexdecimalChars()

			self.activateInterfaceEntry = NoSave(ConfigYesNo(default=iNetwork.getAdapterAttribute(self.iface, "up") or False))

			if not QBOXHD_MINI and ( self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ):
				config.plugins.wlan.active = NoSave(ConfigYesNo(default = iNetwork.check_iNIC_WIFI_Module_active() ))
	...
	def createSetup(self):
		...
		if self.activateInterfaceEntry.value:
			if QBOXHD and not QBOXHD_MINI:
				if ( self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ):
					self.InterfaceWiFiEntry = getConfigListEntry(_("Active WiFi Interface"), config.plugins.wlan.active)
					self.list.append(self.InterfaceWiFiEntry)
			...
			for p in plugins.getPlugins(PluginDescriptor.WHERE_NETWORKSETUP):
				...
				if callFnc is not None:
					if p.__call__.has_key("WlanPluginEntry"): # internally used only for WLAN Plugin
						...
						if QBOXHD:
							if p.__call__.has_key("ifaceActive"):
								self.ifaceActive = p.__call__["ifaceActive"]
							else:
								self.ifaceActive = None

							if not QBOXHD_MINI and self.iface == "eth0" and not config.plugins.wlan.active.value:
								continue
						...
						if config.plugins.wlan.encryption.enabled.value:
							...
							if QBOXHD:
								...
								if config.plugins.wlan.encryption.type.value == 'WEP 64' or config.plugins.wlan.encryption.type.value == 'WEP 128':
									self.wepKeyType = getConfigListEntry(_("Input Key Type"), config.plugins.wlan.encryption.wepkeytype)
									self.list.append( self.wepKeyType )
									if config.plugins.wlan.encryption.wepkeytype.value == 'HEX':
										config.plugins.wlan.encryption.psk.setHexdecimalChars()
	...
	def KeyGreen(self):
		if self.iface == "wlan0" or self.iface == "ath0" or ( QBOXHD and not QBOXHD_MINI and self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ):
	...
	def newConfig(self):
		...
		if self["config"].getCurrent() == self.gatewayEntry:
			self.createSetup()
		if QBOXHD:
			if self.iface == "wlan0" or self.iface == "ath0" or (not QBOXHD_MINI and self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ):

				if self["config"].getCurrent() == self.InterfaceWiFiEntry:
					self.createSetup()
				...
				if self["config"].getCurrent() == self.wepKeyType:
					self.createSetup()
		else: #!QBOXHD
			if self.iface == "wlan0" or self.iface == "ath0" :
				if self["config"].getCurrent() == self.wlanSSID:
	...
	def selectionChanged(self):
		...
		elif not QBOXHD and current == self.encryptionKey and config.plugins.wlan.encryption.enabled.value:
			helpwindowpos = self["HelpWindow"].getPosition()
			if current[1].help_window.instance is not None:
				current[1].help_window.instance.move(ePoint(helpwindowpos[0],helpwindowpos[1]))
				self["ButtonGreen"].show()
	...
	def NetCableDetect_Finished(self, result, retval, extra_args = None):
		if not QBOXHD:
			return
		...
		if (retval==1):
			# netcable detected
			if self.extended != None and config.plugins.wlan.active.value:
				self.session.openWithCallback(None, MessageBox, _("Please disconnect netcable for activate this network configuration"), MessageBox.TYPE_INFO)
				return
		else:
			#netcable not detect
			if not config.plugins.wlan.active.value:
				self.session.openWithCallback(None, MessageBox, _("Please connect netcable and then activate this network configuration."), MessageBox.TYPE_INFO)
				return
	...
	def ok(self):
		if QBOXHD:
			#WLAN Plugin defined ...
			if self.extended != None and ( ( not QBOXHD_MINI and config.plugins.wlan.active.value) or ( QBOXHD_MINI ) ) :
				#if encryption enabled ...
				if config.plugins.wlan.encryption.enabled.value == True:

					if config.plugins.wlan.encryption.type.value == 'WEP 64':
						#validate WEP 64 key encryption HEX
						if config.plugins.wlan.encryption.wepkeytype.value == 'HEX' and len(config.plugins.wlan.encryption.psk.value) != 10 :
							self.session.open(MessageBox, _("For WEP 64 encryption you must specify 5 ascii character or 10 hexadecimal character."), MessageBox.TYPE_ERROR)
							return None

						#validate WEP 64 key encryption ASCII
						if config.plugins.wlan.encryption.wepkeytype.value == 'ASCII' and len(config.plugins.wlan.encryption.psk.value) != 5 :
							self.session.open(MessageBox, _("For WEP 64 encryption you must specify 5 ascii character or 10 hexadecimal character."), MessageBox.TYPE_ERROR)
							return None

					elif config.plugins.wlan.encryption.type.value == 'WEP 128':
						#validate WEP 128 key encryption HEX
						if config.plugins.wlan.encryption.wepkeytype.value == 'HEX' and len(config.plugins.wlan.encryption.psk.value) != 26 :
							self.session.open(MessageBox, _("For WEP 128 encryption you must specify 13 ascii character or 26 hexadecimal character."), MessageBox.TYPE_ERROR)
							return None

						#validate WEP 128 key encryption ASCII
						if config.plugins.wlan.encryption.wepkeytype.value == 'ASCII' and len(config.plugins.wlan.encryption.psk.value) != 13 :
							self.session.open(MessageBox, _("For WEP 128 encryption you must specify 13 ascii character or 26 hexadecimal character."), MessageBox.TYPE_ERROR)
							return None

					elif config.plugins.wlan.encryption.type.value == 'WPA' or config.plugins.wlan.encryption.type.value == 'WPA2':
						#validate WPA WPA2 encryption
						if len(config.plugins.wlan.encryption.psk.value) < 8 or len(config.plugins.wlan.encryption.psk.value) > 63:
							self.session.open(MessageBox, _("For WPA or WPA2 encryption you need to specify 8 to 63 ascii character."), MessageBox.TYPE_ERROR)
							return None
						
			if ( not QBOXHD_MINI and self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ) and self.activateInterfaceEntry.value:
				from Components.Console import Console
				self.NetCableDetectConsole = Console()
				cmd = "qboxhd_netcable_detect"
				self.NetCableDetectConsole.ePopen(cmd, self.NetCableDetect_Finished)
				return
	...
	def applyConfig(self, ret = False):
		if (ret == True):
			...
			if QBOXHD:
				if (not QBOXHD_MINI and self.iface == "eth0" and iNetwork.check_iNIC_WIFI_Module_present() ):
					iNetwork.setAdapterAttribute(self.iface, "wifi_active", config.plugins.wlan.active.value)
				else:
					if self.iface == 'wlan0':
						iNetwork.setAdapterAttribute(self.iface, "wifi_active", True)
			...
			if self.extended is not None and self.configStrings is not None:
				...
				if QBOXHD:
					if self.iface == 'wlan0':
						self.WiFi.writeConfig()
					else:
						if (self.iface == 'eth0' and iNetwork.check_iNIC_WIFI_Module_present() and config.plugins.wlan.active.value):
							self.WiFi.writeConfig()
...
class AdapterSetupConfiguration(Screen, HelpableScreen):
	def __init__(self, session,iface):
		...
		self.oktext = _("Press OK on your remote control to continue.")
		self.reboottext = _("Your QBoxHD will restart after pressing OK on your remote control.")
		self.errortext = _("No working wireless interface found.\n Please verify that you have WiFi module or enable your local network interface.")
		...
		self.onLayoutFinish.append(self.layoutFinished)
		if QBOXHD:
			self.onShown.append(self.updateStatusbar)
		self.onClose.append(self.cleanup)

	def ok(self):
		if self["menulist"].getCurrent()[1] == 'edit':

			if self.iface == 'wlan0' or self.iface == 'ath0' or (QBOXHD and not QBOXHD_MINI and ((self.iface == "eth0") and iNetwork.check_iNIC_WIFI_Module_active()) ):
			#if self.iface == 'wlan0' or self.iface == 'ath0':
				try:
		...
		if self["menulist"].getCurrent()[1] == 'wlanstatus':
			try:
				...
				if self.wlanresponse[0] != 19:
					...
					# Display Wlan not available Message
					self.showErrorMessage()
		if ( QBOXHD and not QBOXHD_MINI and ((self.iface == "eth0") and iNetwork.check_iNIC_WIFI_Module_active()) ) :
			if self["menulist"].getCurrent()[1] == 'antennaposition':
				try:
					from Plugins.SystemPlugins.WirelessLan.plugin import WlanAntennaPositionStatus
					from Plugins.SystemPlugins.WirelessLan.iwlibs import Wireless
				except ImportError:
					self.session.open(MessageBox, _("The wireless LAN plugin is not installed!\nPlease install it."), type = MessageBox.TYPE_INFO,timeout = 10 )
				else:
					...
					self.wlanresponse = ifobj.getStatistics()
					if self.wlanresponse[0] != 19:
						self.session.openWithCallback(self.AdapterSetupClosed, WlanAntennaPositionStatus, self.iface)
	...
	def loadDescription(self):
		if self["menulist"].getCurrent()[1] == 'edit':
			self["description"].setText(_("Edit the network configuration of your QBoxHD.\n" ) + self.oktext )
		if self["menulist"].getCurrent()[1] == 'test':
			self["description"].setText(_("Test the network configuration of your QBoxHD.\n" ) + self.oktext )
		if self["menulist"].getCurrent()[1] == 'dns':
			self["description"].setText(_("Edit the Nameserver configuration of your QBoxHD.\n" ) + self.oktext )
		if self["menulist"].getCurrent()[1] == 'scanwlan':
	...
	def updateStatusbar(self, data = None):
		...
		if QBOXHD:
			if self.iface == 'wlan0' or self.iface == 'ath0' or ( not QBOXHD_MINI and (self.iface == "eth0") and iNetwork.check_iNIC_WIFI_Module_active() ):
				try:
					from Plugins.SystemPlugins.WirelessLan.Wlan import Wlan
					w = Wlan(self.iface)
					stats = w.getStatus()
					
					if len(stats['BSSID'].split(':')) != 6:
	...
	def genMainMenu(self):
		menu = []
		if QBOXHD:
			menu.append((_("Network settings"), "edit"))
		else:
			...
			if callFnc is not None:
				...
				if p.__call__.has_key("WlanPluginEntry"): # internally used only for WLAN Plugin
					if QBOXHD:
						...
						# check if this iface is eth0 and is active wifi bridging ...
						if ( self.ifaceActive is not None and not self.ifaceActive(self.iface)):
							continue
					...
					if iNetwork.getAdapterAttribute(self.iface, "up"):
						...
						if ( QBOXHD and not QBOXHD_MINI and ((self.iface == "eth0") and iNetwork.check_iNIC_WIFI_Module_active()) ):
							menu.append((_("Antenna Position"),"antennaposition"))
				else:
...
class NetworkAdapterTest(Screen):
	...
	def getLinkState(self,iface):
		if QBOXHD:
			if iface == 'wlan0' or iface == 'ath0' or ( not QBOXHD_MINI and (iface == "eth0") and iNetwork.check_iNIC_WIFI_Module_active() ):
#			try:
				from Plugins.SystemPlugins.WirelessLan.Wlan import Wlan
