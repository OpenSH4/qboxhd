...
static const struct dhcp_client_t ext_dhcp_clients[] = {
	...
	{ "udhcpc",
		"udhcpc -R -n -t 10 -p /var/run/udhcpc.%iface%.pid -i %iface%[[ -H %hostname%]][[ -c %clientid%]]"
...
/* qboxhd */
static int wifi_dhcp_up(struct interface_defn_t *ifd, execfn *exec)
{
#if ENABLE_FEATURE_IFUPDOWN_IP
	/* ip doesn't up iface when it configures it (unlike ifconfig) */
	if (!execute("ip link set[[ addr %hwaddress%]] %iface% up", ifd, exec))
...
/* qboxhd */
static int wifi_dhcp_down(struct interface_defn_t *ifd, execfn *exec)
{
	int result = 1;
...
static const struct method_t methods[] = {
	...
	{ "wifi_dhcp", wifi_dhcp_up, wifi_dhcp_down, },
