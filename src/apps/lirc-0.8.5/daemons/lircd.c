...
int waitfordata(long maxusec)
{
	...
	while(1)
	{
		do{
			...
			{
#ifdef QBOXHD
				if( (tv.tv_usec==0) && (tv.tv_sec==0) ) tv.tv_usec=10000;
#endif
			...
			{
				const char *release_message = NULL;
				const char *release_remote_name = NULL;
				const char *release_button_name = NULL;
...
void loop()
{
	...
	while(1)
	{
		...
		if(message!=NULL)
		{
			...
			printf("remote name: %s button_name: %s reps: %d \n", remote_name, button_name, reps);

#ifdef QBOXHD			
			/* so the first 1 repeats are ignored */
			if ( (reps > DELAY_FIRST_REPS) || (reps==0) )
...
#                       endif
		...
		case 'v':
			printf("%s %s\n",progname,VERSION);
			printf("LIRCD Patched ver.%s\n", QBOXHD_PATCH_VERSION );
			return(EXIT_SUCCESS);
		case 'n':
			nodaemon=1;
			break;
