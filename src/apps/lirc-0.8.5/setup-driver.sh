...
# DO NOT EDIT
    ...
    if test "$?" = "0"; then
        ...
        if false; then :
			...
			6 "Awox RF/IR Remote (userspace)" \
			7 "Creative USB IR Receiver (SB0540)" \
			8 "CommandIR Multi-IR Transceiver (userspace)" \
			9 "Dign HV5 HTPC IR/VFD Module" \
			0 "DViCO USB Remote" \
			a "FTDI FT232-based IR Receiver" \
			b "Home Electronics Tira USB device" \
			c "Igor Cesko's USB IR Receiver" \
			d "Iguanaworks USB IR Transceiver" \
			e "IRLink VS USB IR Receiver" \
			f "PCMAK USB receiver" \
			g "Remotec Multimedia PC Remote BW6130" \
			h "Samsung USB IR Receiver" \
			i "Sasem OnAir Remocon-V" \
			j "Sound Blaster Extigy USB sound card (exaudio)" \
			k "Sound Blaster Extigy/Audigy 2 NX (ALSA snd-usb-audio)" \
			l "Soundgraph iMON 2.4G DT & LT" \
			m "Soundgraph iMON MultiMedian IR/VFD" \
			n "Soundgraph iMON IR/LCD" \
			o "Soundgraph iMON Knob" \
			p "Soundgraph iMON PAD IR/VFD" \
			q "Soundgraph iMON RSC" \
			r "Streamzap PC Remote" \
			s "TechnoTrend USB IR receiver" \
			t "USB-UIRT" \
            ...
            if test "$?" = "0"; then
                ...
                elif test "$1" = "6"; then LIRC_DRIVER=awlibusb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "7"; then LIRC_DRIVER=sb0540;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "8"; then LIRC_DRIVER=commandir;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "9"; then LIRC_DRIVER=sasem;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "0"; then LIRC_DRIVER=dvico;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "a"; then LIRC_DRIVER=ftdi;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "b"; then LIRC_DRIVER=tira;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "c"; then LIRC_DRIVER=igorplugusb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "d"; then LIRC_DRIVER=iguanaIR;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "e"; then LIRC_DRIVER=irlink;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "f"; then LIRC_DRIVER=pcmak_usb;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "g"; then LIRC_DRIVER=bw6130;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "h"; then LIRC_DRIVER=samsung;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "i"; then LIRC_DRIVER=sasem;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "j"; then LIRC_DRIVER=exaudio;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "k"; then LIRC_DRIVER=alsa_usb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "l"; then LIRC_DRIVER=imon_24g;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "m"; then LIRC_DRIVER=imon;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "n"; then LIRC_DRIVER=imon_lcd;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "o"; then LIRC_DRIVER=imon_knob;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "p"; then LIRC_DRIVER=imon_pad;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "q"; then LIRC_DRIVER=imon_rsc;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "r"; then LIRC_DRIVER=streamzap;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "s"; then LIRC_DRIVER=ttusbir;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "t"; then LIRC_DRIVER=usb_uirt_raw;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
