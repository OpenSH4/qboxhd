...
#			print "-----> slot.type: " + slot.type + " slot.description: " + slot.description + "\n"
...
#			print "TRANSPONDERS", self.transpondersterrestrial
	...
	def enumerateNIMs(self):
		# enum available NIMs. This is currently very dreambox-centric and uses the /etc/enigma2/bus/nim_sockets interface.
		# the result will be stored into nim_slots.
		# the content of /etc/enigma2/bus/nim_sockets looks like:
		# NIM Socket 0:
		#          Type: DVB-S
		...
		if QBOXHD:
			active_nims = -1
			if entries[0]["name"] != "N/A":
				active_nims = 0
			if entries[1]["name"] != "N/A":
				if active_nims == -1:
					active_nims = 1
				else:
					active_nims = 10
			if not QBOXHD_MINI:
				if entries[2]["name"] != "N/A":
					if active_nims == -1:
						active_nims = 2
					elif active_nims == 0:
						active_nims = 20
			...
			cmd = "/usr/local/bin/testreg 4 " + str(active_nims)
			system(cmd)
	...
	# get a list of only active nim
	def nimActivesList(self):
		list = [ ]
		for slot in self.nim_slots:
			if slot.friendly_full_description.find(_("empty")) == -1:
				list.append(slot.friendly_full_description)
		return list
...
#		print '-----> Number of NIMs: %d\n' % (len(nimmgr.nim_slots))
...
#			section.output_12v = ConfigSelection(choices = [("0V", _("0 V")), ("12V", _("12 V"))], default="0V")
	...
	for slot in nimmgr.nim_slots:
		...
		if slot.isCompatible("DVB-S"):
			...
			if QBOXHD or QBOXHD_MINI:
				if n:
					possible_scan_types = [("provider", _("Provider"))]
