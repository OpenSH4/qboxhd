	...
	def getVideoUrl(self):
		...
		if QBOXHD:
			...
			if mobj is None:
				...
				# 35 = 480p ( SD not available )
				# 18 ( SD only available )
				# 22 = 720p ( HD available )
				# 37 = 1080p ( HD available )
				...
				if len( urls ) > 0 :
					if not isHDAvailable:
						for url in urls:
							...
							url_link, format = url
							if format == '18':
								print " >>> GOT SD URL: %s, format: %s" % ( url_link, format )
								return url_link
					else:
						for url in urls:
							# search for 720p
							url_link, format = url
							if format == '22':
								print " >>> GOT HD URL: %s, format: %s" % ( url_link, format )
								return url_link
								
						#if not available 720p switch to higher resolution
						url_link, format = urls[0]
						print " >>> GOT HD URL: %s, format: %s" % ( url_link, format )
						return url_link
