...
class NFIDownload(Screen):
	...
	def feed_failed(self, failure_instance):
		...
		self["infolabel"].text = _("Could not connect to QBoxHD .NFI Image Feed Server:") + "\n" + failure_instance.getErrorMessage() + "\n\n" + _("Please check your network settings!")
	...
	def dmesg_cleared(self, answer):
		...
		self.msgbox = self.session.open(MessageBox, _("Please disconnect all USB devices from your QBoxHD and (re-)attach the target USB stick (minimum size is 64 MB) now!"), MessageBox.TYPE_INFO)
	...
	def backup_finished(self, skipped=False):
		if not skipped:
			...
			if wizardfd:
				...
				wizardfd.close()
		self.session.open(MessageBox, _("To update your QBoxHD firmware, please follow these steps:\n1) Turn off your box with the rear power switch and plug in the bootable USB stick.\n2) Turn mains back on and hold the DOWN button on the front panel pressed for 10 seconds.\n3) Wait for bootup and follow instructions of the wizard."), type = MessageBox.TYPE_INFO)
