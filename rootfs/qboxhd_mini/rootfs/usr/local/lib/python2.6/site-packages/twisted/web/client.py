...
def downloadPage2(url, file, contextFactory=None, *args, **kwargs):
    ...
    @param file: path to file on filesystem, or file-like object.
    See HTTPDownloader to see what extra args can be passed.
    Modified version of the twisted.web.client::downloadPage() function.
    Returns also the HTTPDownloader object, not just the Deferrer.
    """
    scheme, host, port, path = _parse(url)
    ...
    if scheme == 'https':
        ...
        if contextFactory is None:
            contextFactory = ssl.ClientContextFactory()
        clientConn=reactor.connectSSL(host, port, factory, contextFactory)
    else:
        clientConn=reactor.connectTCP(host, port, factory)

    return factory.deferred, factory, clientConn	
