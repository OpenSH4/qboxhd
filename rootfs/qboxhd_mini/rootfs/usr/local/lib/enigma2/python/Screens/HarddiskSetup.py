...
class HarddiskWait(Screen):
	...
	def __init__(self, session, hdd, type):
		...
		if type == HarddiskSetup.HARDDISK_INITIALIZE:
			text = _("Initializing Harddisk... \nThis operation can take several minutes depending on the Harddisk size...")
...
class HarddiskSetup(Screen):
	...
	def hddReady(self, result):
		...
		if (result != 0):
			...
			if QBOXHD:
				if self.type == self.HARDDISK_INITIALIZE:
					if QBOXHD:
						message = _("Harddisk formatted.\nSuccessfully")
					else:
						message = _("Initialize harddisk.\nSuccessfully")
				else:
					message = _("Filesystem check.\nSuccessfully")
	...
	def hddQuestion(self):
		if self.type == self.HARDDISK_INITIALIZE:
			if QBOXHD:
				message = _("Do you really want to format the harddisk?\nAll data on the disk will be lost!")
			else:
