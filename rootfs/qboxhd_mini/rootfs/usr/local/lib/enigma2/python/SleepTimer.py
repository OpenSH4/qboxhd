...
if QBOXHD_MINI:
	config.SleepTimer.action = ConfigSelection(default = "deep standby", choices = [("deep standby", _("deep standby")), ("standby", _("standby"))])
else:
			...
			if QBOXHD_MINI:
				if config.SleepTimer.action.value == "deep standby":
					if config.SleepTimer.ask.value and not Screens.Standby.inTryQuitMainloop:
						Notifications.AddNotificationWithCallback(self.shutdown, MessageBox, _("A sleep timer wants to deep standby\nyour QBoxHD. Deep standby now?"), timeout = 20)
					else:
						self.shutdown(True)
