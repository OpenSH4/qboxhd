...
def loadSkin(name, scope = SCOPE_SKIN):
	...
	#dom_skins.append((mpath, cElementTree.parse(filename).getroot()))
...
################################################			
		elif attrib == "separator":
			pix = gseparator
			if pix.find("#cached") == -1:
				pix += "#cached"
			ptr = loadPixmap(gseparator, desktop)
			guiObject.setPixmap(ptr)
#################################################			
		...
		elif QBOXHD and attrib == "requires":
			return
		else:
	...
	for widget in myscreen.findall("widget"):
		...
		if wname:
			...
			try:
				...
				if QBOXHD:
					print SkinError("component with name '" + wname + "' was not found in skin of screen '" + name + "'!")
					continue
...
#			assert screen[wname] is not Source
			...
			while True: # until we found a non-obsolete source
				...
				while len(path) > 1:
					...
					if scr is None:
						...
						#print name
						if QBOXHD:
							print SkinError("specified related screen '" + wsource + "' was not found in screen '" + name + "'!")
							continue
			...
			if source is None:
				if QBOXHD:
					print SkinError("source '" + wsource + "' was not found in screen '" + name + "'!")
					continue
			...
			if not wrender:
				if QBOXHD:
					print SkinError("you must define a renderer with render= for source '%s'" % (wsource))
					continue
