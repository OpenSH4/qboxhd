...
class SleepTimerEdit(Screen):
	def __init__(self, session):
		...
		if QBOXHD_MINI:
			self["pretext"] = Label(_("Deep Standby QBoxHD after"))
		else:
			self["pretext"] = Label(_("Shutdown QBoxHD after"))
	...
	def updateColors(self):
		...
		if QBOXHD_MINI:
			if config.SleepTimer.action.value == "deep standby":
			...
			if config.SleepTimer.ask.value:
				self["yellow_text"].setText(_("Ask before deep standby:") + " " + _("yes"))
			else:
				self["yellow_text"].setText(_("Ask before deep standby:") + " " + _("no"))
				
				
		else:
			if config.SleepTimer.action.value == "shutdown":
				self["green_text"].setText(_("Sleep timer action:") + " " + _("ShutDown"))
			elif config.SleepTimer.action.value == "standby":
				self["green_text"].setText(_("Sleep timer action:") + " " + _("Standby"))
	...
	def toggleAction(self):
		if QBOXHD_MINI:
			if config.SleepTimer.action.value == "deep standby":
				...
				config.SleepTimer.action.value = "deep standby"
		else:
			if config.SleepTimer.action.value == "shutdown":
				config.SleepTimer.action.value = "standby"
			elif config.SleepTimer.action.value == "standby":
