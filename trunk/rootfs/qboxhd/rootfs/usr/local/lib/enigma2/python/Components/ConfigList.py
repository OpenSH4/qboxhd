...
class ConfigListScreen:
	...
	def keySave(self):
		self.saveAll()
		if QBOXHD:
			self.session.openWithCallback(self.keySaveCB, MessageBox, _("Your new configuration have been saved successfully."), type = MessageBox.TYPE_INFO)
		else:
			self.close()
			
	def cancelConfirmCB(self, result):
		for x in self["config"].list:
			x[1].cancel()
		self.close()
	...
	def cancelConfirm(self, result):
		...
		if QBOXHD:
			self.session.openWithCallback(self.cancelConfirmCB, MessageBox, _("No configuration saved."), type = MessageBox.TYPE_INFO)
		else:
			if not result:
				return
