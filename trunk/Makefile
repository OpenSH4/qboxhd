...
help:
	...
	@echo "    Build and install frontends drivers for the QBoxHD:"
	@echo "        make BOARD=qboxhd frontends"
	@echo "        make BOARD=qboxhd frontends_install"
	@echo "    Build enigma2:"
...
PLAYER2_COMMAND=make -C $(SRC_KERNEL)  O=$(BUILD_KERNEL) M=$(BUILD_PLAYER2)/linux ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) INSTALL_MOD_PATH=$(ROOTFS) PLAYER2_PATH=$(SRC_PLAYER2) INCLUDE_PATH=$(SRC_INCLUDE) STMFB_PATH=$(SRC_STMFB) FE_PATH=$(SRC_FE) PTI_PATH=$(SRC_PTI) AVS_PATH=$(SRC_AVS) DELAYER_PATH=$(SRC_DELAYER) EXTRA_CFLAGS="-Idrivers/media/dvb/dvb-core"

PTI_COMMAND=make -C $(SRC_KERNEL) O=$(BUILD_KERNEL) M=$(BUILD_PTI) ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) INSTALL_MOD_PATH=$(ROOTFS) PLAYER2_PATH=$(SRC_PLAYER2) INCLUDE_PATH=$(SRC_INCLUDE) EXTRA_CFLAGS="-Idrivers/media/dvb/dvb-core -DDEBUG_DVB -D__TDT__" 

FE_COMMAND=make -C $(SRC_KERNEL) O=$(BUILD_KERNEL) M=$(BUILD_FE) ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) INSTALL_MOD_PATH=$(ROOTFS) PLAYER2_PATH=$(SRC_PLAYER2) INCLUDE_PATH=$(SRC_INCLUDE) BOARD_TYPE=$(BOARD) EXTRA_CFLAGS="-Idrivers/media/dvb/dvb-core -DDEBUG_DVB -D__TDT__"
...
DELAYER_COMMAND=make -C $(SRC_KERNEL) O=$(BUILD_KERNEL) M=$(BUILD_DELAYER) ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) INSTALL_MOD_PATH=$(ROOTFS) PLAYER2_PATH=$(SRC_PLAYER2) INCLUDE_PATH=$(SRC_INCLUDE) EXTRA_CFLAGS="-Idrivers/media/dvb/dvb-core"
...
modules_install: .check_board .modules_install_kernel stmfb_install multicom_install player2_install frontends_install avs_install e2proc_install lcd_install lpc_install fpanel_install smartcard_install scart_install qboxhd_generic_install starci2win_install
...
modules_clean: .check_board .modules_clean_kernel stmfb_clean multicom_clean player2_clean frontends_clean avs_clean e2proc_clean lcd_clean lpc_clean fpanel_clean smartcard_clean scart_clean qboxhd_generic_clean starci2win_clean
