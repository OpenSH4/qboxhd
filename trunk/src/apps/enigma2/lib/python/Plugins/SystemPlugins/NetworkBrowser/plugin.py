...
def Plugins(path, **kwargs):
	...
	return [
		PluginDescriptor(name=_("NAS Server Configuration"), description=_("Search for network shares"), where = PluginDescriptor.WHERE_NETWORKSETUP, fnc={"ifaceSupported": NetworkBrowserCallFunction, "menuEntryName": lambda x: _("NAS Server Configuration"), "menuEntryDescription": lambda x: _("Search for network shares...")})
	]
