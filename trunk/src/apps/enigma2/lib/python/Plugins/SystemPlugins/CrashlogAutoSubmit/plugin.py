...
config.plugins.crashlogautosubmit.name = ConfigText(default = "QBoxHD User", fixed_size = False)
...
class CrashlogAutoSubmitConfiguration(Screen, ConfigListScreen):
	...
	def selectionChanged(self):
		...
		if current == self.MailEntry:
			...
			self["status"].setText(_("Adds enigma2 settings and QBoxHD model informations like SN, rev... if enabled."))
			self.disableVKeyIcon()
...
def mxServerFound(mxServer,session):
	...
	if str(config.plugins.crashlogautosubmit.name.value) ==  "QBoxHD User":
		user_name = ""
	else:
