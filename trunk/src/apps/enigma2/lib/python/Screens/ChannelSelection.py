	...
	def recallBouquetMode(self):
		if self.mode == MODE_TV:
			...
			if config.usage.multibouquet.value:
				...
				if QBOXHD:
					self.bouquet_rootstr = '%s FROM BOUQUET "bouquets.tv" ORDER BY bouquet'%(self.service_types)
				else:
			...
			if config.usage.multibouquet.value:
				...
				if QBOXHD:
					self.bouquet_rootstr = '%s FROM BOUQUET "bouquets.radio" ORDER BY bouquet'%(self.service_types)
				else:
...
class ChannelSelection(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, SelectionEventInfo):
	def __init__(self, session):
		...
		#patch: block zap while recording
