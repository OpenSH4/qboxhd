...
###		
	if QBOXHD:
		config.usage.hdd_swap = ConfigYesNo(default = False)
...
###
	...
	if QBOXHD:
		def setSwapHDD(configElement):
			if configElement.value:
				print "Enable All SWAP Partitions"
				harddiskmanager.enableAllSwapPartition()
				print "SWAP Partitions Enabled"
			else:
				print "Disable All SWAP Partitions"
				harddiskmanager.disableAllSwapPartition()
				print "SWAP Partitions Disabled"
			configfile.save()
			
		config.usage.hdd_swap.addNotifierSave(setSwapHDD)
	...
	config.seek.speeds_forward = ConfigSet(default=[2, 4, 8], choices=[2, 4, 6, 8])
	config.seek.speeds_backward = ConfigSet(default=[2, 4, 8], choices=[1, 2, 4, 6, 8])
	config.seek.speeds_slowmotion = ConfigSet(default=[2, 4, 8], choices=[2, 4, 6, 8, 12, 16, 25])

	#config.seek.enter_forward = ConfigSelection(default = "2", choices = ["2", "4", "6", "8", "12", "16", "24", "32", "48", "64", "96", "128"])
	#config.seek.enter_backward = ConfigSelection(default = "1", choices = ["1", "2", "4", "6", "8", "12", "16", "24", "32", "48", "64", "96", "128"])
	config.seek.enter_forward = ConfigSelection(default = "2", choices = ["2", "4", "6", "8"])
	config.seek.enter_backward = ConfigSelection(default = "1", choices = ["1", "2", "4", "6", "8"])
	...
	if QBOXHD:
		def updateSystemLogsStatus(configElement):
			from os import system
			if not configElement.value:
				print "System logs disabled"
				system("echo 0 > /proc/sys/kernel/printk")
			else:
				print "System logs enabled"
				system("echo 8 > /proc/sys/kernel/printk")
		
		config.systemlogs = ConfigYesNo(default = False)
		config.systemlogs.addNotifier(updateSystemLogsStatus, immediate_feedback = False)
