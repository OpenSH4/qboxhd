...
RESULT eNavigation::playService(const eServiceReference &service)
{
	...
	eDebug("eNavigation::playService: res=%d",res);
	if(res)
	{
		eDebug("eNavigation::playService: FAILURE!!!!");
		return res;
	}

	if (m_runningService)
	{
		eDebug("eNavigation::playService: m_runningService");
...
RESULT eNavigation::recordService(const eServiceReference &ref, ePtr<iRecordableService> &service, bool simulate)
{
	...
	eDebug("record: res=%d, simulate=%s", res,simulate?"TRUE":"FALSE");
	if (res)
		service = 0;
	else
	{
