...
std::string eActionMap::getLastRCId() 
{ 
	return last_rc_id;
}
...
#ifdef QBOXHD
void eActionMap::bindKey(const std::string &domain, const std::string &device, const std::string &rc_id, int key, int flags, int repeat_for_event, const std::string &context, const std::string &action)
#else
...
#ifdef QBOXHD
			bind.m_rc_id = rc_id;
			bind.m_repeat_for_event = repeat_for_event;
#endif
			bind.m_key = key;
			bind.m_flags = flags;
...
#ifdef QBOXHD
void eActionMap::keyPressed(const std::string &device, const std::string &rc_id, int key, int flags)
#else
		...
		if (c->second.m_widget)
		{
			...
			if (c->second.m_context.size())
			{
				...
				for (; k != e; ++k)
				{
					...
					{
#ifdef QBOXHD
//						eDebug("Flags: %d k->second.m_flags: %d\n", flags, k->second.m_flags);

						/* If there you press another button i reset its m_repeat_counter and flags is repeated */
						if ( (k->second.m_repeat_for_event>0) && ( flags == eRCKey::flagMake ) ) {
//							eDebug("---------> Reset m_repeat_counter\n");
							m_repeat_counter = 0;
						}

						last_rc_id = rc_id;

						/* Discard key if the rc has a requires tag in keymap.xml */
						if ( (strlen(k->second.m_rc_id.c_str()) > 0) && 
							 (strcmp(k->second.m_rc_id.c_str(),"None") != 0 ) && 
							 ( k->second.m_rc_id != rc_id ) 
						   ) {
							//eDebug( "Discard keymap %s != %s\n", k->second.m_rc_id.c_str(), rc_id.c_str() );
							continue;
						}

						/* If there is enable a m_repeat_for_event i check if m_repeat_counter is equal to m_repeat_for_event. */
						if ( (k->second.m_repeat_for_event>0)  && ( k->second.m_flags & 4 ) && ( flags & eRCKey::flagRepeat) ) {
							m_repeat_counter++;
//							eDebug("---------> m_repeat_counter %d\n", m_repeat_counter);
							if ( ( m_repeat_counter>0 ) && ( m_repeat_counter % k->second.m_repeat_for_event ) != 0 )
							{
								continue;
							}
							else {
//								eDebug( "Generate event after m_repeat_counter=%d  m_repeat_for_event=%d %s \n", m_repeat_counter, k->second.m_repeat_for_event, k->second.m_rc_id.c_str() );
							}	
						}
#endif
