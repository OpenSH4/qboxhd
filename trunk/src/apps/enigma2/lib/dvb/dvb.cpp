...
{
	m_demux->m_inuse++;
	eDebug("eDVBAllocatedDemux::eDVBAllocatedDemux() - m_demux->m_inuse=%d",m_demux->m_inuse);
}
...
#endif
		eDebug("eDVBAdapterLinux::eDVBAdapterLinux(): frontend - filename=%s",filename);
		if (stat(filename, &s))
			break;
...
#endif
// 		eDebug("eDVBAdapterLinux::eDVBAdapterLinux(): demux - filename=%s",filename);
		if (stat(filename, &s))
			break;
...
// 	eDebug("allocate demux");
	eDebug("eDVBResourceManager::allocateDemux() - allocate demux [m_demux.size()=%d]",m_demux.size());
	...
	if (i == m_demux.end())
	{
		eDebug("eDVBResourceManager::allocateDemux() - empty m_demux list => return -1");
		return -1;
	}
	...
	{
		if (fe)
		{//retrieve demux for live streams
			if (!i->m_inuse && (i->m_adapter == fe->m_adapter))
			{
				if (!unused)
				{
					unused = i;
					eDebug("eDVBResourceManager::allocateDemux() with fe - n=%d",n);
					break;        /// FOUND!!! May I break the loop?
				}
			}
				...
				i->m_demux->getSource() == fe->m_frontend->getDVBID())
			{
				...
				eDebug("eDVBResourceManager::allocateDemux() with fe - ALLOCATED DMX - n=%d",n);
				return 0;
			}
		}
		else
		{//retrieve demux for PVR streams
			if (n == (m_demux.size()-1) ) // always use demuxN-1 for PVR
			{
				eDebug("eDVBResourceManager::allocateDemux() without fe - n=%d",n);
				if (i->m_inuse)
				{
					demux = new eDVBAllocatedDemux(i);
					eDebug("eDVBResourceManager::allocateDemux() without fe - ALLOCATED DMX - n=%d",n);
					return 0;
				}
				unused = i;
				break;
	...
	if (m_demux.size() < 5)
	{
		...
		for (; i != m_demux.end(); ++i, ++n)
		{
			...
			eDebug("eDVBResourceManager::allocateDemux() - is_decode=%d,in_use=%d n=%d",is_decode,in_use,n);

			if ((!in_use) && ((!fe) || (i->m_adapter == fe->m_adapter)))
			{
				if ((cap & iDVBChannel::capDecode) && !is_decode)
					continue;
				eDebug("eDVBResourceManager::allocateDemux() - unused=i n=%d",n);
				unused = i;
				break;
			}
		}
...
// eDebug("eDVBAllocatedDemux::allocatedDemux() - we assume dm8000 - n=%d",n);
		for (; i != m_demux.end(); ++i, ++n)
		{
			if (fe)
			{
	...
	if (unused)
	{
		...
		if (fe)
		{
			eDebug("eDVBResourceManager::allocateDemux(): setSource FE");
...
#ifdef QBOXHD
			eDebug("eDVBResourceManager::allocateDemux(): setSourcePVR(%d) [demux=/dev/dvb/adapter%d/demux%d]",
				   pvrSource,demux->get().getAdapter(),demux->get().getDemux());
			{
				char dmxPath[30]="";

				sprintf(dmxPath,"/dev/dvb/adapter%d/dvr%d",demux->get().getAdapter(),demux->get().getDemux());
				unlink("/dev/misc/pvr");
				if(symlink(dmxPath, "/dev/misc/pvr"))
				{   //at first time misc dir could miss
					mkdir("/dev/misc", 0x0777);
					if(symlink(dmxPath, "/dev/misc/pvr"))
						eDebug("eDVBResourceManager::allocateDemux(): cannot make symlink current PVR demux");
				}
			}
#endif

			if(demux->get().setSourcePVR(pvrSource)<0)
			{
				eDebug("eDVBResourceManager::allocateDemux(): setSourcePVR(%d)",pvrSource);
				return -1;
			}
		}
		return 0;
	...
	eDebug("eDVBResourceManager::allocateDemux(): demux not found");
	return -1;
}
...
{
	int state=0;
	eDebug("eDVBResourceManager::DVBChannelStateChanged() - called");
	chan->getState(state);
	switch (state)
	{
...
eDebug("eDVBChannel::frontendStateChanged(): called");
...
void eDVBChannel::cueSheetEvent(int event)
{
	...
	case eCueSheet::evtSeek:
		eDebug("eDVBChannel::cueSheetEvent():  evtSeek.");
// 		eDebug("seek.");
		flushPVR(m_cue->m_decoding_demux);
		break;
	case eCueSheet::evtSkipmode:
	{
		eDebug("eDVBChannel::cueSheetEvent():  evtSkipmode.");
		{
			m_cue->m_lock.WrLock();
	...
	{
		eDebug("eDVBChannel::cueSheetEvent():  evtSpanChanged.");
		m_source_span.clear();
...
void eDVBChannel::getNextSourceSpan(off_t current_offset, size_t bytes_read, off_t &start, size_t &size)
{
	...
	if (!m_cue)
	{
		eDebug("eDVBChannel::getNextSourceSpan(): no cue sheet. forcing normal play");
// 		eDebug("no cue sheet. forcing normal play");
	...
	if (!m_cue->m_decoding_demux)
	{
		start = current_offset;
		size = max;
		eDebug("eDVBChannel::getNextSourceSpan(): no decoding demux. forcing normal play");
		m_cue->m_lock.Unlock();
		return;
	}
	...
	for (std::list<std::pair<off_t, off_t> >::const_iterator i(m_source_span.begin()); i != m_source_span.end(); ++i)
	{
		...
		if ((current_offset >= aligned_start) && (current_offset < aligned_end))
		{
			...
			eDebug("eDVBChannel::getNextSourceSpan(): HIT, %lld < %lld < %lld, size: %d", i->first, current_offset, i->second, size);
			return;
		}
		if (current_offset < aligned_start)
		{
			...
			eDebug("eDVBChannel::getNextSourceSpan(): result: %llx, %x (%llx %llx)", start, size, aligned_start, aligned_end);
			return;
		}
	}
...
{
// 	eDebug("eDVBChannel::getDemux() - called with cap=%d  <-----------------------",cap);
	...
	if (!our_demux)
	{
		...
		{
			eDebug("eDVBChannel::getDemux() - FAILED");
			return -1;
		}
		demux = *our_demux;
...
{
	eDebug("eDVBChannel::playFile(%s) to m_pvr_fd_dst=%d",file,m_pvr_fd_dst);
	...
	if (m_pvr_fd_dst < 0)
	{
...
#ifdef QBOXHD
		/// PATCH: try to solve problem on return from TS-play to live.
		int adapterIndex=0;
		int dmxIndex=0;
		int dvrIndex=0;
		char dvrDev[128];
		 ///MY code patch lets to choose a demux on the same adapter will be choosen later on state change
		ePtr<iDVBDemux> demux;

		if (getDemux(demux,(int)iDVBChannel::capDecode))
		{
			eDebug("eDVBChannel::playFile(): Allocating a decoding demux for play file failed.");
			return -ENODEV;
		}

		adapterIndex = demux->getAdapter();
		dmxIndex= demux->getDemux();
		dvrIndex=dmxIndex;

		///This will always be adapterX/dvr0
		eDebug("eDVBChannel::playFile() adapter=%d demux=%d dvr=%d",adapterIndex,dmxIndex,dvrIndex);
		sprintf(dvrDev, "/dev/dvb/adapter%d/dvr%d", adapterIndex,dvrIndex);
		...
		if (m_pvr_fd_dst < 0)
		{
			...
			eDebug("eDVBChannel::playFile(): opened %s", dvrDev);
	}
	...
	eDebug("eDVBChannel::playFile() starting play '%s' to m_pvr_fd_dst=%d",file,m_pvr_fd_dst);
	...
	eDebug("eDVBChannel::playFile() started PLAY\n");
	CONNECT(m_pvr_thread->m_event, eDVBChannel::pvrEvent);

	m_state = state_ok;
	m_stateChanged(this);

	eDebug("eDVBChannel::playFile() DONE.");
	return 0;
}
...
void eDVBChannel::flushPVR(iDVBDemux *decoding_demux)
{
	...
	eDebug("eDVBChannel::flushPVR(): CALLING m_pvr_thread->resume();");
	m_pvr_thread->resume();
	eDebug("eDVBChannel::flushPVR(): CALLED m_pvr_thread->resume();");
}
