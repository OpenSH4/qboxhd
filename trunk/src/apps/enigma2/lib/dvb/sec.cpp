...
// eDebug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>eDVBSatelliteEquipmentControl::canTune.");
...
// eDebug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>eDVBSatelliteEquipmentControl::canTune. DONE.\n\n");
	return score;
}
		...
		if ( sit != lnb_param.m_satellites.end())
		{
			...
			// calc Frequency
				local = abs(sat.frequency 
					- ((lof - (lof % 1000)) + ((lof % 1000)>500 ? 1000 : 0)) ); //TODO f�r den Mist mal ein Macro schreiben
...
RESULT eDVBSatelliteEquipmentControl::clear()
{
	...
	for (eSmartPtrList<eDVBRegisteredFrontend>::iterator it(m_avail_frontends.begin()); it != m_avail_frontends.end(); ++it)
	{
		...
		{
#ifdef QBOXHD
			FILE *f=fopen("/etc/enigma2/stb/tsmux/lnb_b_input", "w");
			if (!f || fwrite("B", 1, 1, f) != 1)
				eDebug("set /etc/enigma2/stb/tsmux/lnb_b_input to B failed!! (%m)");
			else
			{
				eDebug("set /etc/enigma2/stb/tsmux/lnb_b_input to B OK");
				fclose(f);
			}
#else
		...
		if (p1 && p2)
		{
			...
			if (!strcmp(p1->m_frontend->getDescription(), p2->m_frontend->getDescription()) && !strcmp(p1->m_frontend->getDescription(), "BCM4501 (internal)"))
			{
				...
				if (!f || fwrite("A", 1, 1, f) != 1)
					eDebug("set /etc/enigma2/stb/tsmux/lnb_b_input to A failed!! (%m)");
				else
				{
					eDebug("set /etc/enigma2/stb/tsmux/lnb_b_input to A OK");
					fclose(f);
				}
#else
				FILE *f=fopen("/proc/stb/tsmux/lnb_b_input", "w");
