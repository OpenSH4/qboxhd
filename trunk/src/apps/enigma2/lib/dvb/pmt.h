...
class eDVBCAService: public Object
{
...
#ifdef QBOXHD
	unsigned char m_adapterId;
	unsigned int m_frequency;
	unsigned int m_orbital_position;
	...
	int comunicate_generic_info(const char *buf);	
	static unsigned char m_toggle_frontend;
	void timeoutCAPMT();
#endif
	void sendCAPMT();
	void Connect();
...
#ifdef QBOXHD
	void buildCAPMT(eTable<ProgramMapSection> *ptr,unsigned char adapterId=0xff);
#else
