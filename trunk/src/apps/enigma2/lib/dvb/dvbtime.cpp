...
{
#ifdef QBOXHD_MINI
	FILE *f = fopen("/proc/stb/lpc/rtc", "w");
#else
...
#ifdef QBOXHD_MINI
			eDebug("write /proc/stb/lpc/rtc failed (%m)");
#else
...
time_t getRTC()
{
	...
	FILE *f = fopen("/proc/stb/lpc/rtc", "r");
	if (fscanf(f, "%u %u", &tmp,&tmp_s) != 1)
		eDebug("read /proc/stb/lpc/rtc failed (%m)");
	fclose(f);
/*
	t1->mjd-h, t2->mjd-l,t3->hour-bcd, t4->min-bcd, t5->sec-bcd, hash->set 0
*/
	eDebug("\n\nThe date and time from rtc-lpc -> tmp:%u tmp_s:%u",tmp,tmp_s);
	eDebug("\t\trtc-lpc -> tmp:%08X tmp_s:%08X",tmp,tmp_s);
	rtc_time=parseDVBtime(((tmp>>24)&0xFF), ((tmp>>16)&0xFF),((tmp>>8)&0xFF),(tmp&0xFF), tmp_s,0);
	eDebug("RTC in time_t: 0x%08X\n\n",rtc_time);
	...
	:m_use_dvb_time(false), m_updateNonTunedTimer(eTimer::create(eApp)), m_time_ready(false)
{
	...
	else
	{
		...
		{
// 			eDebug("\n\nSystem time not ok....try with rtc of lpc");
			now = getRTC();
// 			eDebug("----> now: 0x%08X -- %d",now,now);
			
			if ( now > 1072224000 ) // 01.01.2004
			{
				eDebug("Setting system time!\n\n");
				stime(&now);
			}
		}
#endif			
