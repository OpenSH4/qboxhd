...
#ifdef QBOXHD
// We do not support ioctl DMX_ADD_PID
// #undef HAVE_ADD_PID
#define HAVE_ADD_PID
#else
#define HAVE_ADD_PID
...
{
	//eDebug("eDVBDemux::eDVBDemux(adapter=%d,demux=%d)  ---------------- \n",adapter,demux);
	m_dvr_busy = 0;
}
...
int eDVBDemux::openDemux(void)
{
	...
	eDebug("eDVBDemux::openDemux(void) - filename='%s'",filename);
...
RESULT eDVBDemux::setSourcePVR(int pvrnum)
{
	...
	int res = ::ioctl(fd, DMX_SET_SOURCE, &n);
	if (res)
		eDebug("eDVBDemux::setSourcePVR(%d) - ioctl(fd, DMX_SET_SOURCE, %d) - res=%d - failed! - %m",pvrnum,n,res);
	else
		eDebug("eDVBDemux::setSourcePVR(%d) - ioctl(fd, DMX_SET_SOURCE, %d) - res=%d",pvrnum,n,res);
...
{
//	eDebug("eDVBDemux::createTSRecorder - called \n");

	if (m_dvr_busy)
	{
		eDebug("eDVBDemux::createTSRecorder -  m_dvr_busy=%d\n",m_dvr_busy);
		return -EBUSY;
	}
	recorder = new eDVBTSRecorder(this);

	eDebug("eDVBDemux::createTSRecorder - DONE.");
	return 0;
}
...
eDVBPESReader::eDVBPESReader(eDVBDemux *demux, eMainloop *context, RESULT &res): m_demux(demux)
{
	...
	{
#ifdef QBOXHD
		eDebug("eDVBPESReader::eDVBPESReader(): DMX_SET_BUFFER_SIZE to 8*64*1024");
		::ioctl(m_fd, DMX_SET_BUFFER_SIZE, 8*64*1024);
// 		::ioctl(m_fd, DMX_SET_BUFFER_SIZE, 128*1024);
#else
...
RESULT eDVBPESReader::start(int pid)
{
	...
	if (res)
		eWarning("eDVBPESReader::start(): PES filter: DMX_SET_PES_FILTER - %m");
	if (!res)
		m_active = 1;
	return res;
}
...
#endif
#ifndef QBOXHD
	//init values for timeshift recording
	m_timeshift_enabled = 0;
	m_timeshift_filename = std::string("");
#endif
}
...
RESULT eDVBTSRecorder::start()
{
	...
	eDebug("eDVBTSRecorder::start() - opened dvr (%s) with fd=%d", filename, m_source_fd);

#ifndef QBOXHD
	if(m_timeshift_enabled==1)
{
	int buffSize = 4*1880*1024;// 256*1024;
	if (::ioctl(m_source_fd, DMX_SET_BUFFER_SIZE, buffSize) < 0)
		eDebug("DMX_SET_BUFFER_SIZE failed(%m)");
	else
		eDebug("eDVBTSRecorder::start() - DMX_SET_BUFFER_SIZE set to %d",buffSize);
}
#endif // QBOXHD
...
// 	int buffSize = 2*1880*1024; //256*1024;

	int buffSize = 4*1880*1024;
	if (::ioctl(m_source_fd, DMX_SET_BUFFER_SIZE, buffSize) < 0)
		eDebug("DMX_SET_BUFFER_SIZE failed(%m)");
	else
		eDebug("eDVBTSRecorder::start() - DMX_SET_BUFFER_SIZE set to %d",buffSize);
...
#if 0
	// Code from trunk-release old version
	flt.pes_type = (dmx_pes_type_t)DMX_TAP_TS;
	flt.pid     = (_u16) -1;
	flt.input   = DMX_IN_FRONTEND;
	flt.output  = DMX_OUT_TAP;
	...
	m_thread->start(m_source_fd, m_target_fd); //records data from m_source_fd to m_target_fd
	m_running = 1;
...
RESULT eDVBTSRecorder::addPID(int pid)
{
	...
	eDebug("eDVBTSRecorder::addPID( %04x) m_running=%d", pid,m_running);
	if (m_running)
		startPID(pid);
	return 0;
}
...
{
// eDebug("eDVBTSRecorder::stop() - called.\n");
...
// 	close(m_source_fd);
	::close(m_source_fd);
// eDebug("eDVBTSRecorder::stop() - closed fd m_source_fd==%d.\n",m_source_fd);
	m_source_fd = -1;
...
{
	eDebug("eDVBTSRecorder::startPID(%04x) >", pid);
#ifndef HAVE_ADD_PID
	...
	while(true) {
		...
		if (::ioctl(m_source_fd, DMX_ADD_PID, pid) < 0) {
			...
			m_pids[pid] = 1;
		eDebug("eDVBTSRecorder::startPID(%04x) breaking", pid);
		break;
	}
#endif
	eDebug("eDVBTSRecorder::startPID(%04x) DONE.<", pid);
	return 0;
}
