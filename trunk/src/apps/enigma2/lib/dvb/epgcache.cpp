...
next:
		...
		{
#ifdef QBOXHD
			FILE *f = fopen("/etc/enigma2/event_map.txt", "w+");
#else /// !QBOXHD
			...
			fclose(f);
#ifdef QBOXHD
			f = fopen("/etc/enigma2/time_map.txt", "w+");
#else /// !QBOXHD
...
void eEPGCache::load()
{
	...
	if (f)
	{
		...
		{
#ifdef QBOXHD
			FILE *f = fopen("/etc/enigma2/epg.dat.md5", "r");
#else /// !QBOXHD
...
#ifdef QBOXHD
                // Python 2.4 -> 2.6
                argcount = (((PyStringObject*)(obj))->ob_size);
#else
...
#ifdef QBOXHD
                    // Python 2.4 -> 2.6
                    int textlen = (((PyStringObject*)(obj))->ob_size);
#else
#if PY_VERSION_HEX < 0x02060000
