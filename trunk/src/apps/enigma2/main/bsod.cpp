...
/************************************************/
#ifdef QBOXHD
#define CRASH_EMAILADDR "support@duolabs.com"
#else
...
void bsodFatal(const char *component)
{
	...
	if (f)
	{
		...
		std::string pythonmd5 = execCommand((char*)"find /usr/lib/enigma2/python/ -name \"*.py\" | xargs md5sum");
...

	{
		...
		char text[512];
#ifdef QBOXHD
		snprintf(text, 512, "We are really sorry. Your QBoxHD encountered "
							"a software problem, and needs to be restarted. "
							"Please send the logfile created in /hdd/ to %s.\n"
							"Your QBoxHD restarts in 10 seconds!\n"
							"Component: %s",
...
#ifndef QBOXHD //do not raise SIGKILL to avoid signal handler loops
	raise(SIGKILL);
	eDebug("bsodFatal: raise(SIGKILL)");
#endif
}
