...
#################################################################
	...
	def activate(self):
		...
		if next_state == self.StatePrepared:
			...
			if QBOXHD_MINI and nimmanager.getSlotCount()<2 and len(NavigationInstance.instance.getRecordings()) :
				self.log(5, "there is already a record running")
				notifications.AddPopup(text = _("Timer cannot start.\nA recording is currently running.\nNo more recording allowed."), type = MessageBox.TYPE_INFO, timeout = 3)
				self.state = RecordTimerEntry.StateEnded
				return False
