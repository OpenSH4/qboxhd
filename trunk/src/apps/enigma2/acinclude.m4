...
AC_ARG_WITH(qboxhd,
    [  --without-qboxhd         disable support for QBoxHD],
    [QBOXHD="$withval"],[QBOXHD="yes"])

AC_ARG_WITH(qboxhd_mini,
    [  --with-qboxhd-mini         enable support for QBoxHD mini],
    [QBOXHD_MINI="$withval"],[QBOXHD_MINI="no"])

if test "$QBOXHD" = "yes"; then
	if test "$QBOXHD_MINI" = "yes"; then
		AC_DEFINE(QBOXHD,1,[Enable support for QBoxHD])
		AC_DEFINE(QBOXHD_MINI,1,[Enable support for QBoxHD mini])
		AC_SUBST([QBOXHDBOARD],["-DQBOXHD -DQBOXHD_MINI"])
		printf 'QBOXHD = 1\nQBOXHD_MINI = 1\nfrom Components.SystemInfo import SystemInfo\nSystemInfo[["QBoxHD_mini"]] = True\n' >  qboxhd.py
	else
		AC_DEFINE(QBOXHD,1,[Enable support for QBoxHD])
		AC_SUBST([QBOXHDBOARD],["-DQBOXHD"])
		printf 'QBOXHD = 1\nQBOXHD_MINI = 0\nfrom Components.SystemInfo import SystemInfo\nSystemInfo[["QBoxHD"]] = True' >  qboxhd.py
	fi
fi
...
AC_DEFINE(HAVE_DVB_API_VERSION,5,[Enable DVB API 5])
...
AC_DEFUN([AC_PYTHON_DEVEL],[
	...
	if test $cross_compiling = "yes"; then
		...
		# QBoxHD workaround
		QBOXHD_PYTHON_CPPFLAGS="-I/opt/STM/STLinux-2.3/devkit/sh4/target/usr/local/include"
		python_path=
		for i in $QBOXHD_PYTHON_CPPFLAGS ; do
			p=`echo $i | sed "s,^-I,,"`
		...
		# QBoxHD workaround
		QBOXHD_PYTHON_LDFLAGS="-L/opt/STM/STLinux-2.3/devkit/sh4/target/usr/local/lib/python2.6 -L/opt/STM/STLinux-2.3/devkit/sh4/target/usr/local/lib/python2.6/site-packages"
