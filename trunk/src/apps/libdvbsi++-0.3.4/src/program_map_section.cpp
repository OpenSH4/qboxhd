...
ProgramMapSection::ProgramMapSection(const uint8_t * const buffer) : LongCrcSection(buffer)
{
	...
	printf("*********************************** sectionLength %08x\n", sectionLength ); 

	printf("*************************************** DVB_PID(&buffer[8]) = %04x\n", DVB_PID(&buffer[8]));
#endif
	...
	printf("*************************************** DVB_LENGTH(&buffer[10]) = %04x\n", DVB_LENGTH(&buffer[10]));
#endif
	...
	uint16_t bytesLeft2 = 0;

	bytesLeft2 = programInfoLength;
