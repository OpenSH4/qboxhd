...
#define LIRC_STM_ENABLE_IRQ		0x1f
#else
...
/*
 * Software SCD support.
 * SCD data could be passed by lircd...
 */
struct lirc_filter {
	const char *name;
	const lirc_t *scd;
	int count;
	const struct lirc_filter *next;
};

static const struct lirc_filter *lirc_filter;
static char lirc_filter_name[16];

/* ruwido timings: STA + CustomID + Address */
static const lirc_t ruwido_scd[] = {
	213, 426, 213, 213, 426, 426, 213, 213, 213, 213, 213, 213
};

static const struct lirc_filter ruwido_filter = {
	.name	= "ruwido",
	.scd	= ruwido_scd,
	.count	= ARRAY_SIZE(ruwido_scd),
};

/* nec timings: AGC burst + space + Address */
static const lirc_t nec_scd[] = {
	9000, 2250
};

static const lirc_t nec_rep_scd[] = {
	9000, 4500
};

static const struct lirc_filter nec_rep_filter = {
	.scd	= nec_rep_scd,
	.count	= ARRAY_SIZE(nec_rep_scd),
};

static const struct lirc_filter nec_filter = {
	.name	= "nec",
	.scd	= nec_scd,
	.count	= ARRAY_SIZE(nec_scd),
	.next	= &nec_rep_filter,
};

static const struct lirc_filter *lirc_filters[] = {
	&ruwido_filter,
	&nec_filter,
};

static __init void lirc_filter_init(void)
{
	int i;

	if (lirc_filter_name[0] == '\0')
		return;		/* no filtering */

	for (i = 0; i < ARRAY_SIZE(lirc_filters); i++) {
		if (strcmp(lirc_filters[i]->name, lirc_filter_name))
			continue;
		lirc_filter = lirc_filters[i];
		return;
	}
	/* make this the ST way... */
	printk(KERN_WARNING LIRC_STM_NAME
	       ": '%s' unknown, no filtering.\n", lirc_filter_name);
	lirc_filter_name[0] = '\0';
}

/* this match lircd.conf */
#define LIRC_SYMBOL_MAX(x)	((x) + (x) * 4 / 10)
#define LIRC_SYMBOL_MIN(x)	((x) - (x) * 4 / 10)

static int lirc_data_match_one(lirc_stm_rx_data_t *rx,
			       const struct lirc_filter *filter)
{
	const lirc_t *scd = filter->scd;
	int count = filter->count / 2;
	int i;

	if (rx->off_rbuf < count)
		return 0;

	for (i = 0; i < count; i++) {
		unsigned pulse = rx->rbuf[i*2] & ~PULSE_BIT;
		unsigned space = rx->rbuf[i*2 + 1];

		if (pulse < LIRC_SYMBOL_MIN(scd[i*2]) ||
		    pulse > LIRC_SYMBOL_MAX(scd[i*2]))
			break;
		if (space < LIRC_SYMBOL_MIN(scd[i*2+1]) ||
		    space > LIRC_SYMBOL_MAX(scd[i*2+1]))
			break;
	}
	return i == count;
}

static int lirc_data_match(lirc_stm_rx_data_t *rx)
{
	const struct lirc_filter *f;

	for (f = lirc_filter; f != NULL; f = f->next) {
		if (lirc_data_match_one(rx, f))
			return 1;
	}
	return !lirc_filter;
}
...
{
	extern void stb710x_fake_resume(int fake);
...
#if defined(CONFIG_HW_WYBOX) && defined(CONFIG_PM)
	stb710x_fake_resume(1);
#endif
	while (RX_WORDS_IN_FIFO()) {
		...
		if ((mark > 2) && (symbol > 1)) {
			...
			if (lastSymbol) {
				 ...
				 */
				if (lirc_data_match(&rx) &&
				    likely(lirc_buffer_available
					   (&lirc_stm_rbuf) >=
...
module_param_string(filter, lirc_filter_name, sizeof lirc_filter_name, 0444);
MODULE_PARM_DESC(filter, "Name of the filter to use, by default none");
