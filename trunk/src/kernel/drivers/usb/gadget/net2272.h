...
static inline void __iomem *net2272_port_base(struct net2272 *dev, unsigned reg)
{
	return dev->base_addr + (reg << dev->shift_addr);
}
 ...
 */
static inline void net2272_write(struct net2272 *dev, unsigned int reg, u8 value)
{
	if (reg < dev->indexed_threshold) {
		writeb(value, net2272_port_base(dev, reg));
		return;
	}
	writeb((u8)reg, net2272_port_base(dev, REGADDRPTR));
	writeb(value, net2272_port_base(dev, REGDATA));
}

static inline u8 net2272_read(struct net2272 *dev, unsigned int reg)
{
	if (reg < dev->indexed_threshold) {
		return readb(net2272_port_base(dev, reg));
	}
	writeb((u8)reg, net2272_port_base(dev, REGADDRPTR));
	return readb(net2272_port_base(dev, REGDATA));
}

/*
 * Assume we're using 16 bits mode by default
 */
static inline u16 net2272_read_data(struct net2272 *dev)
{
	return readw(net2272_port_base(dev, EP_DATA));
}

static inline void net2272_write_data(struct net2272 *dev, u16 data)
{
	writew(data, net2272_port_base(dev, EP_DATA));
}

static inline void net2272_writeb_data(struct net2272 *dev, u8 data)
{
	u8 tmp = net2272_read(dev, LOCCTL);

	net2272_write(dev, LOCCTL, tmp & ~(1 << DATA_WIDTH));
	writeb(data, net2272_port_base(dev, EP_DATA));
