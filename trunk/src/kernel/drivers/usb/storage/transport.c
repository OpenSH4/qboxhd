...
static int usb_stor_msg_common(struct us_data *us, int timeout)
{
	...
	long timeleft;
	int status;
#if defined CONFIG_SH_QBOXHD_1_0 || CONFIG_SH_QBOXHD_MINI_1_0
	//int i = 0;
#endif
...
#if defined CONFIG_SH_QBOXHD_1_0 || CONFIG_SH_QBOXHD_MINI_1_0
	//if (us->current_urb->status < 0 && us->current_urb->status != -EINPROGRESS) {
		//if (++i == 50) {
			//usb_unlink_urb(us->current_urb);
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	//printk("\t\t\t\t=====> %s(): bcs->Tag: %d, us->tag: %d\n", __func__, bcs->Tag, us->tag);
	...
	US_DEBUGP("Bulk Status S 0x%x T 0x%x R %u Stat 0x%x\n",
			...
			residue, bcs->Status);
#if defined CONFIG_SH_QBOXHD_1_0 || CONFIG_SH_QBOXHD_MINI_1_0
	/* The Command Status Wrapper (CSW) tag must be the same as the Command Bulk Wrapper (CBW) tag */
	//if (bcs->Tag != us->tag) {
		//printk("\t\t\t\t=====> %s(): Forcing Tag!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", __func__);
		//bcs->Tag = us->tag;
		//}
#endif
