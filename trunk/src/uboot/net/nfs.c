...
static struct rpc_t rpc_pkt;
static int rpc_pktlen;
static int sport;
...
rpc_req (int rpc_prog, int rpc_proc, uint32_t *data, int datalen)
{
	...
	rpc_pktlen = (char *)p + datalen*sizeof(uint32_t) - (char *)&rpc_pkt;

	memcpy ((char *)NetTxPacket + NetEthHdrSize() + IP_HDR_SIZE, (char *)&rpc_pkt, rpc_pktlen);
	...
	NetSendUDPPacket (NetServerEther, NfsServerIP, sport, NfsOurPort, rpc_pktlen);
}
