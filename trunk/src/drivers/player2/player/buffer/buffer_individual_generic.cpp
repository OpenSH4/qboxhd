...
//
    ...
    memset( AttachedBuffers, 0x00, MAX_BUFFER_OWNER_IDENTIFIERS * sizeof(unsigned int) );
...
BufferStatus_t   Buffer_Generic_c::ExpandBuffer( unsigned int	  NewSize )
{
    AssertNonZeroReferenceCount( "Buffer_Generic_c::ExpandBuffer" );

    return Pool->ExpandMemoryBlock( BufferBlock, NewSize );
}
...
#if 0
    if( BufferBlock->Descriptor->Type == 0x10 )
	report( severity_info, "AZA - Inc %2d - %d\n", Index, ReferenceCount );
#endif
...
#if 0
    if( BufferBlock->Descriptor->Type == 0x10 )
	report( severity_info, "AZA - Dec %2d - %d\n", Index, ReferenceCount );
#endif
