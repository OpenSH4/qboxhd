...
OutputTimerStatus_t   OutputTimer_Base_c::Reset(	void )
{
    ...
    ForceTrickModeReEvaluation			= false;
...
//
    ...
    Player->GetCodedFrameBufferPool( Stream, &CodedFrameBufferPool );
...
unsigned long long	  DurationEnd;
    ...
    //
    // Update the group structure information
    //
    ...
    Player->GetPresentationInterval( Playback, &StartInterval, &EndInterval );
...
report( severity_info, "Frame - %6lld %6lld %6lld - %6lld %d %d - %6lld\n", 
...
static unsigned int 	  HistoryIndex	= 0;
    ...
    ExternalMappingPolicy	= Player->PolicyValue( Playback, Stream, PolicyExternalTimeMapping );
    ErrorThreshhold		= (ExternalMappingPolicy == PolicyValueApply) ?
					Configuration.SynchronizationErrorThresholdForExternalSync :
					Configuration.SynchronizationErrorThreshold;
...
//
	...
	case SyncStateConfirmedSyncError:
	    ...
	    //
	    // Synchronization corrections can often mess up the output rate integration, so we restart it here.
	    //
...
Rational_t	MaxSubstandardRate;
    ...
    //
    // Obtain the parameters from the codec
    //

    if( TrickModeParameters == NULL )
    {
    ...
    //
    // Calculate usefule numbers
    //

    MaxNormalRate				= TrickModeParameters->MaximumNormalDecodeRate;
    MaxSubstandardRate				= TrickModeParameters->SubstandardDecodeSupported ? 
							TrickModeParameters->MaximumSubstandardDecodeRate : 
							TrickModeParameters->MaximumNormalDecodeRate;
    ...
    TrickModeDomainTransitToStartDiscard	= (MaxSubstandardRate * TrickModeGroupStructureNonReferenceFrames) +
						  (MaxNormalRate      * TrickModeGroupStructureReferenceFrames);

    TrickModeDomainTransitToDecodeReference	= MaxNormalRate / TrickModeGroupStructureReferenceFrames;

#if 0
    TrickModeDomainTransitToDecodeKey		= ((MaxNormalRate * TrickModeGroupStructureIndependentFrames) + 
						   (MaxSubstandardRate * (TrickModeGroupStructureReferenceFrames - TrickModeGroupStructureIndependentFrames))) /
							(TrickModeGroupStructureReferenceFrames * TrickModeGroupStructureReferenceFrames);
#else
    // Nick changed this because we cannot apply substandard decode to reference frames
    TrickModeDomainTransitToDecodeKey		= TrickModeDomainTransitToDecodeReference; 
#endif
...
PlayerEventRecord_t	  Event;
    ...
    TrickModeParametersChanged	= (TrickModeParameters->MaximumNormalDecodeRate != LastMaximumNormalDecodeRate);
    ...
    if( TrickModeParametersChanged || GroupStructureChanged || ForceTrickModeReEvaluation )
	SetTrickModeDomainBoundaries();
	...
	{
	    report( severity_info, "OutputTimer_Base_c::TrickModeControl - Domain = %d, PortionToLose = %4d.%06d, Adjusted speed = %4d.%06d\n",
			TrickModeDomain,
	...
	LastMaximumNormalDecodeRate			= TrickModeParameters->MaximumNormalDecodeRate;
	ForceTrickModeReEvaluation			= false;
    }
...
PlayerEventRecord_t		Event;
	...
	case AccumulatedDecodeInTimeFailures:
				...
				if( DecodeBuffersInUse >= (DecodeBufferCount - 1) )
				{
				    ...
				    ((CodedFrameBuffersInUse < ((2 * CodedFrameBufferCount) / 3)) &&
					(CodedFrameBufferCount < DecodeInTimeCodedDataLevel)) )
				{
				    //
				...
				else if( FailedBy > DecodeInTimeBehind )
				{
				    ...
				    Policy	= Player->PolicyValue( Playback, Stream, PolicyLowerCodecDecodeLimitsOnFailureToDecodeInTime );
				    if( Policy == PolicyValueApply )
				    {
				    	TrickModeParameters->MaximumNormalDecodeRate		= Rational_t(9, 10) * TrickModeParameters->MaximumNormalDecodeRate;
				    	TrickModeParameters->MaximumSubstandardDecodeRate	= Rational_t(9, 10) * TrickModeParameters->MaximumSubstandardDecodeRate;

					if( TrickModeParameters->MaximumNormalDecodeRate < 1 )
					    TrickModeParameters->MaximumNormalDecodeRate	= 1;

					if( TrickModeParameters->SubstandardDecodeSupported &&
					    (TrickModeParameters->MaximumSubstandardDecodeRate < 1) )
					    TrickModeParameters->MaximumSubstandardDecodeRate	= 1;

					report( severity_info, "OutputTimer_Base_c::DecodeInTimeFailure(%s) - Lowered codec decode limits to %d.%06d and %d.%06d\n",
						Configuration.OutputTimerName,
						TrickModeParameters->MaximumNormalDecodeRate.IntegerPart(), TrickModeParameters->MaximumNormalDecodeRate.RemainderDecimal(),
						TrickModeParameters->MaximumSubstandardDecodeRate.IntegerPart(), TrickModeParameters->MaximumSubstandardDecodeRate.RemainderDecimal() );

				    	ForceTrickModeReEvaluation	= true;
				    }
