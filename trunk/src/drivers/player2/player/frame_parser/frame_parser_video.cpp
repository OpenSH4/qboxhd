...
FrameParserStatus_t     Status;
...
#if 0 ///defined(CONFIG_SH_QBOXHD_1_0) || defined(CONFIG_SH_QBOXHD_MINI_1_0)
	if( Status == FrameParserCodecError )
	{
		error_h264_header++;
		report( severity_error, " ##### Error in header read\n");
	...
	if(error_h264_header>MAX_H264_HEADER_ERROR)
	{
		error_h264_header=0;
		report( severity_error, " ##### A lot header error ... Maybe the codec is wrong\n");
...
//
    ...
    if( (BytesUsed + 4) > AntiEmulationContent )        // NOTE add 4 to avoid a show(32) overrunning
    {
