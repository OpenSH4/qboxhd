...
// 	printk("HavanaPlayback_c::%s: Setting speed to %d.%06d\n", __FUNCTION__, Speed.IntegerPart(), Speed.RemainderDecimal());
...
// 		printk("HavanaPlayback_c::%s: Failed to set speed - Status = %x\n", __FUNCTION__, Status);
		return HavanaError;
	}
	return HavanaNoError;
}
