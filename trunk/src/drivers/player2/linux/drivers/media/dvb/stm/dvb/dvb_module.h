...
*/
#define DVB_DEBUG(fmt, args...)         { do { if (ENABLE_DVB_DEBUG && dvb_debug) printk(KERN_INFO"%s: " fmt, __FUNCTION__,##args); } while (0);}
...
struct DvbContext_s
{
	...
	struct DvbContext_s **DvbContextArray;
	struct DvbCurrDecoder_s **DvbCurrDecoderArray;
};

struct DvbCurrDecoder_s
{
	struct DvbContext_s *DvbContext;
};
